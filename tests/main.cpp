...
static bool CopyOne( const std::string& src, const std::string& dst )
{
  ...
  if ( !is )
  {
    fprintf( stderr, "STAGE: FAIL open src [%s]\n", src.c_str() );
    return false;
  }
  std::ofstream os( Normalize( dst ), std::ios::binary );
  if ( !os )
  {
    fprintf( stderr, "STAGE: FAIL open dst [%s]\n", dst.c_str() );
    return false;
  }
  os << is.rdbuf();
  bool ok = os.good();
  if ( ok )
    fprintf( stderr, "STAGE: OK  %s\n", src.c_str() );
  else
    fprintf( stderr, "STAGE: FAIL write %s\n", dst.c_str() );
  return ok;
}
