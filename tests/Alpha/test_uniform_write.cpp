...
TEST( UniformWriteTest, IsTriviallyCopyable ) { EXPECT_TRUE( std::is_trivially_copyable_v<UniformWrite> ); }
...
TEST( SamplerWriteTest, IsTriviallyCopyable ) { EXPECT_TRUE( std::is_trivially_copyable_v<SamplerWrite> ); }
...
TEST( SamplerWriteTest, SizeIsSane ) { EXPECT_GE( sizeof( SamplerWrite ), 12u ); }
