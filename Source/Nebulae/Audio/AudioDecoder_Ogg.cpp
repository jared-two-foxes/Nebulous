...
const ov_callbacks oggVorbis_callbacks = { file_read, ogg_file_seek, file_close, file_tell };


OggAudioDecoder::OggAudioDecoder() : current_section( 0 ) {}

OggAudioDecoder::~OggAudioDecoder() { ov_clear( &vf ); }

void OggAudioDecoder::Clear()
///
...
bool OggAudioDecoder::Parse( File* datastream )
///
...
bool OggAudioDecoder::Seek( Real time )
///
...
double OggAudioDecoder::Tell() const
///
...
double OggAudioDecoder::GetDuration() const
///
...
int32 OggAudioDecoder::Stream( uint32 start, uint32 size, char* data )
///
...
///
{
  ...
  while ( totalread < size )
  {
    ...
    else if ( actualread == OV_HOLE || actualread == OV_EBADLINK || actualread == OV_EINVAL )
    { // ERROR
      if ( ++errorcount >= 3 )
      {
        break;
