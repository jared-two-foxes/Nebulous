...
AudioSoundInterface* AudioEmitterInterface::GetPlayingSound() const
{
  return m_queuedSounds.empty() ? nullptr : m_queuedSounds.front().sound;
}


void AudioEmitterInterface::Update( const Real elapsed )
///
...
Real AudioEmitterInterface::GetPlayCursor() const
///
...
void AudioEmitterInterface::SetPlayCursor( Real time )
///
...
void AudioEmitterInterface::Push( AudioSoundInterface* sound, int32 loops, bool force )
///
...
void AudioEmitterInterface::Pop()
///
...
bool AudioEmitterInterface::StreamBuffer()
///
...
///
{
  ...
  else if ( bytes > 0 )
  {
    ...
    {                                // Shit. Something went wrong binding that data to the buffer?
