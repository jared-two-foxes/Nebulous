...
OverlayRenderer::OverlayRenderer( RenderSystemPtr renderSystem, SpriteBatch* batcher )
  : m_renderDevice( renderSystem ), m_basicMaterial( nullptr ), m_complexMaterial( nullptr ), m_batcher( batcher )
{
}
...
void OverlayRenderer::DrawQuad( RenderSystemPtr renderer, const Nebulae::Point& upperLeft,
                                const Nebulae::Point& lowerRight, const Colour& colour, const SubTexture* subtexture,
                                float depth, float rotation ) const
{
  if ( m_batcher )
  {
...
void OverlayRenderer::DrawComplexQuad( RenderSystemPtr renderer, const Nebulae::Point& ul, const Nebulae::Point& ur,
...
void OverlayRenderer::Rectangle( RenderSystemPtr renderer, Nebulae::Point ul, Nebulae::Point lr, Colour colour,
...
void OverlayRenderer::FlatRectangle( Point ul, Point lr, Colour color, Colour borderColor, uint32 borderThick ) const
{
  Rectangle( m_renderDevice, ul, lr, color, borderColor, borderColor, borderThick, true, true, true, true );
}

void OverlayRenderer::BeveledRectangle( Point ul, Point lr, Colour color, Colour borderColor, bool up,
                                        uint32 bevelThick, bool bevelLeft, bool bevelTop, bool bevelRight,
                                        bool bevelBottom ) const
{
  Rectangle( m_renderDevice, ul, lr, color, ( up ? GetLightColour( borderColor ) : GetDarkColour( borderColor ) ),
             ( up ? GetDarkColour( borderColor ) : GetLightColour( borderColor ) ), bevelThick, bevelLeft, bevelTop,
             bevelRight, bevelBottom );
}

Colour OverlayRenderer::GetLightColour( Colour clr ) const
{
  const double scale_factor = 2.0; // factor by which the color is lightened
  Colour retval = clr;
  retval.r = static_cast<unsigned char>(
    std::min( static_cast<int>( retval.r * scale_factor ), 255 ) ); // int-to-uchar, clamped to [0,255]
  retval.g = static_cast<unsigned char>(
    std::min( static_cast<int>( retval.g * scale_factor ), 255 ) ); // int-to-uchar, clamped to [0,255]
  retval.b = static_cast<unsigned char>(
    std::min( static_cast<int>( retval.b * scale_factor ), 255 ) ); // int-to-uchar, clamped to [0,255]
  return retval;
}

Colour OverlayRenderer::GetDarkColour( Colour clr ) const
{
  const double scale_factor = 2.0; // factor by which the color is darkened
  Colour retval = clr;
  retval.r =
    static_cast<unsigned char>( static_cast<int>( retval.r / scale_factor ) ); // int-to-uchar, result is in [0,127]
  retval.g =
    static_cast<unsigned char>( static_cast<int>( retval.g / scale_factor ) ); // int-to-uchar, result is in [0,127]
  retval.b =
    static_cast<unsigned char>( static_cast<int>( retval.b / scale_factor ) ); // int-to-uchar, result is in [0,127]
  return retval;
}

Colour OverlayRenderer::GetDisabledColour( Colour clr ) const
{
  Colour retval = clr;
  const double gray_factor = 0.75; // amount to move clr in the direction of gray
  retval.r = static_cast<unsigned char>(
    static_cast<int>( retval.r + ( CLR_GRAY.r - retval.r ) * gray_factor ) ); // int-to-uchar, result is in [0,255]
  retval.g = static_cast<unsigned char>(
    static_cast<int>( retval.g + ( CLR_GRAY.g - retval.g ) * gray_factor ) ); // int-to-uchar, result is in [0,255]
  retval.b = static_cast<unsigned char>(
    static_cast<int>( retval.b + ( CLR_GRAY.b - retval.b ) * gray_factor ) ); // int-to-uchar, result is in [0,255]
  return retval;
}
