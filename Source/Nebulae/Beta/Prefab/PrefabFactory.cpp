...
PlaneLoader::PlaneLoader( RenderSystem& renderDevice ) : m_renderDevice( renderDevice ) {}

PlaneLoader::~PlaneLoader() {}

Geometry* PlaneLoader::Create( float width, float depth, int rows, int columns )
{
...
void PlaneLoader::UpdateIndexBuffer( HardwareBuffer* ib, int rows, int columns )
{
...
CubeLoader::CubeLoader( RenderSystem& renderDevice ) : m_renderDevice( renderDevice ) {}

CubeLoader::~CubeLoader() {}

Geometry* CubeLoader::Create( float width, float height, float depth, int rows, int columns, int slices )
{
...
void CubeLoader::UpdateVertexBuffer( HardwareBuffer* vb, int /*rows*/, int /*columns*/, int /*slices*/, float /*dx*/,
                                     float /*dy*/, float /*dz*/ )
{
  const int NUM_VERTICES = 4 * 6; // 4 vertices per side * 6 sides
  const int NUM_ENTRIES_PER_VERTEX = 6;
...
void CubeLoader::UpdateIndexBuffer( HardwareBuffer* ibuf, int /*rows*/, int /*columns*/, int /*slices*/ )
{
  ...
  // Create index buffer
  uint16 indexScratch[NUM_INDICES] = { 3,  1,  0,  2,  1,  3,
