...
Real RandFloat( const Real min, const Real max ) { return min + ( ( float( rand() ) / RAND_MAX ) * ( max - min ) ); }


ParticleEmitter::ParticleEmitter( const std::string& name )
  : m_name( name ), m_active( false ), m_position( 0, 0, 0 ), m_rotation( 0, 0, 0, 1 )
{
}
...
void ParticleEmitter::Pause() { m_active = false; }

void ParticleEmitter::Update( const uint64 elapsed )
{
...
void ParticleEmitter::SetPosition( const Vector4& position ) { m_position = position; }

void ParticleEmitter::SetRotation( const Quaternion& rotation ) { m_rotation = rotation; }

void ParticleEmitter::SetEmissiveForce( const Distribution<Vector4>& force ) { m_emissionForce = force; }

const Vector4& ParticleEmitter::GetPosition() const { return m_position; }

const Quaternion& ParticleEmitter::GetRotation() const { return m_rotation; }
...
bool ParticleEmitter::IsActive() const { return m_active; }

void ParticleEmitter::AddParticlesToReservoir( ParticleGroup* group, uint32 count, Real flow )
{
