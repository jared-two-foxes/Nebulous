...
int Widget::GetID() const { return m_id; }


const std::string& Widget::GetName() const { return m_name; }


bool Widget::IsInteractive() const { return ( m_flags & INTERACTIVE ) != 0; }


bool Widget::RepeatButtonDown() const { return ( m_flags & REPEAT_BUTTON_DOWN ) != 0; }


bool Widget::IsDragable() const { return ( m_flags & DRAGABLE ) != 0; }


bool Widget::IsResizable() const { return ( m_flags & RESIZABLE ) != 0; }


bool Widget::IsOnTop() const { return !m_parent && ( ( m_flags & ONTOP ) != 0 ); }


bool Widget::IsModal() const { return !m_parent && ( ( m_flags & MODAL ) != 0 ); }
...
const std::string& Widget::GetDragDropDataType() const { return m_dragDropDataType; }

void Widget::DropsAcceptable( DropsAcceptableIter first, DropsAcceptableIter last, const Point& /*pt*/ ) const
{
...
int Widget::GetWidth() const { return ( m_lowerRight.x - m_upperLeft.x ); }


int Widget::GetHeight() const { return ( m_lowerRight.y - m_upperLeft.y ); }
...
int Widget::GetClientWidth() const { return GetClientLowerRight().x - GetClientUpperLeft().x; }


int Widget::GetClientHeight() const { return GetClientLowerRight().y - GetClientUpperLeft().y; }
...
bool Widget::InWindow( const Point& pt ) const { return pt >= GetUpperLeft() && pt < GetLowerRight(); }


bool Widget::InClient( const Point& pt ) const { return pt >= GetClientUpperLeft() && pt < GetClientLowerRight(); }


const std::list<Widget*>& Widget::GetChildren() const { return m_children; }


Widget* Widget::GetParent() const { return m_parent; }


Widget* Widget::GetRootParent() const
{
...
int Widget::GetDepth() const { return m_zorder; }


const WidgetFactory& Widget::GetWidgetFactory() const { return m_widgetFactory; }


WidgetRegion Widget::GetWidgetRegion( const Point& pt ) const
{
  enum
  {
    LEFT = 0,
...
void Widget::SetDragDropDataType( const std::string& dataType ) { m_dragDropDataType = dataType; }


void Widget::StartingChildDragDrop( const Widget* /*wnd*/, const Point& /*offset*/ ) {}


void Widget::AcceptDrops( const std::vector<Widget*>& /*widgets*/, const Point& /*pt*/ ) {}


void Widget::CancellingChildDragDrop( const std::vector<const Widget*>& /*widgets*/ ) {}
...
void Widget::SetName( const std::string& name ) { m_name = name; }


void Widget::Hide( bool children )
{
  m_visible = false;
  if ( children )
  {
...
void Widget::MoveTo( const Point& pt ) { SizeMove( pt, pt + GetSize() ); }


void Widget::OffsetMove( const Point& pt ) { SizeMove( m_upperLeft + pt, m_lowerRight + pt ); }


void Widget::SizeMove( const Point& ul_, const Point& lr_ )
{
  Point ul = ul_, lr = lr_;
...
void Widget::Resize( const Point& sz ) { SizeMove( m_upperLeft, m_upperLeft + sz ); }


void Widget::SetMinSize( const Point& sz )
{
...
void Widget::SetMaxSize( const Point& sz )
{
  m_maxSize = sz;
...
void Widget::MoveChildUp( Widget* widget )
{
  if ( widget )
  {
...
void Widget::MoveChildDown( Widget* widget )
{
  if ( widget )
  {
    if ( std::find( m_children.begin(), m_children.end(), widget ) != m_children.end() )
...
void Widget::InstallEventFilter( Widget* widget )
{
  if ( widget )
  {
...
void Widget::RemoveEventFilter( Widget* widget )
{
  if ( widget )
  {
...
void Widget::TouchDown( const Point& /*pt*/, Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
...
void Widget::TouchDrag( const Point& /*pt*/, const Point& move, Flags<ModKey> /*mod_keys*/ )
{
  if ( IsDragable() )
  {
    OffsetMove( move );
...
void Widget::TouchUp( const Point& /*pt*/, Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
    ForwardEventToParent();
...
void Widget::Clicked( const Point& /*pt*/, Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
    ForwardEventToParent();
...
void Widget::DragDropEnter( const Point& /*pt*/, const std::map<Widget*, Point>& /*dragDropWidgets*/,
...
void Widget::DragDropHere( const Point& /*pt*/, const std::map<Widget*, Point>& /*dragDropWidgets*/,
                           Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
...
void Widget::KeyPress( KeyCode /*key*/, uint32 /*key_code_point*/, Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
    ForwardEventToParent();
...
void Widget::KeyRelease( KeyCode /*key*/, uint32 /*key_code_point*/, Flags<ModKey> /*mod_keys*/ )
{
  if ( !IsInteractive() )
  {
    ForwardEventToParent();
...
bool Widget::EventFilter( Widget* /*widget*/, const WidgetEvent& /*event*/ ) { return false; }


void Widget::ForwardEventToParent()
{
