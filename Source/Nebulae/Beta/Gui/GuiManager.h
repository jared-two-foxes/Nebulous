...
public:
  ...
  uint64 GetDoubleClickInterval()
    const; ///< returns the maximum interval allowed between clicks that is still considered a double-click, in ms
  ...
  bool IsDragDropWidget( const Widget* widget )
    const; ///< returns true if \a Widget is currently begin dragged as part of a drag-and-drop operation
  bool IsAcceptedDragDropWidget(
    const Widget* widget ) const; ///< returns true if \a Widget is currently begin dragged as part of a drag-and-drop
                                  ///< operation, and it is over a drop target that will accept it
  ...
  bool HandleEvents( EventType eventType, KeyCode key, uint32 key_code_point, Flags<ModKey> mod_keys, const Point& pos,
                     const Point& rel );
