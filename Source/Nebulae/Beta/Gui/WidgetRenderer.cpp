...
FontRenderer* WidgetRenderer::AddFont( const std::shared_ptr<Font>& font )
{
...
void WidgetRenderer::SetFocusWindow( Widget* focusWidget ) { m_focusWidget = focusWidget; }

void WidgetRenderer::Render( const std::list<Widget*>& widgets )
///
...
const FontRenderer* WidgetRenderer::FindFontRendererByFont( const std::shared_ptr<Font>& font ) const
{
...
void WidgetRenderer::BeginClipping( const Widget& widget )
{
  // switch( mode ) {
  //   case DontClip:
...
void WidgetRenderer::EndClipping( const Widget& /*widget*/ )
{
  // switch (mode) {
  //   case DontClip:
...
void WidgetRenderer::RenderButton( const ButtonControl& control, int32 depth )
{
  ...
  else
  {
    BeveledRectangle( control.GetUpperLeft(), control.GetLowerRight(),
                      ...
                      control.IsDisabled() ? GetDisabledColour( control.GetColour() ) : control.GetColour(),
                      ( control.GetState() != ButtonControl::BN_PRESSED ),
                      static_cast<uint32>( 1 ) ); // int-to-uint32, small constant
  }
...
void WidgetRenderer::RenderTextControl( const TextControl& control, int32 depth )
{
  ...
  if ( control.GetFont() != NULL )
  {
    ...
    fontRenderer->RenderText( control.GetUpperLeft(), control.GetLowerRight(), control.GetText(), text_format,
                              clr_to_use, &line_data );
...
void WidgetRenderer::RenderEditControl( const EditControl& control, int32 depth )
{
  Colour color_to_use = control.IsDisabled() ? GetDisabledColour( control.GetColour() ) : control.GetColour();
  Colour int_color_to_use =
    control.IsDisabled() ? GetDisabledColour( control.InteriorColour() ) : control.InteriorColour();
  Colour sel_text_color_to_use =
    control.IsDisabled() ? GetDisabledColour( control.SelectedTextColour() ) : control.SelectedTextColour();
  Colour hilite_color_to_use =
    control.IsDisabled() ? GetDisabledColour( control.HiliteColour() ) : control.HiliteColour();
  ...
  BeveledRectangle( ul, lr, color_to_use /*int_color_to_use*/, color_to_use, false,
                    static_cast<uint32>( 2 ) ); // int-to-uint32, small constant
  ...
  int32 text_y_pos( static_cast<int32>( ul.y + ( ( lr.y - ul.y ) - control.GetFont()->Height() ) / 2.0 +
                                        0.5 ) ); // double-to-int32, truncation intended for pixel alignment
  ...
  if ( control
         .MultiSelected() ) // if one or more chars are selected, hilite, then draw the range in the selected-text color
  {
    ...
    // draw hiliting
    Point hilite_ul(
      client_ul.x + ( low_cursor_pos < 1 ? 0 : char_data[Value( low_cursor_pos - 1 )].extent ) - first_char_offset,
      client_ul.y );
    Point hilite_lr( client_ul.x + char_data[Value( high_cursor_pos - 1 )].extent - first_char_offset, client_lr.y );
    FlatRectangle( hilite_ul, hilite_lr, hilite_color_to_use, CLR_ZERO,
                   static_cast<uint32>( 0 ) ); // int-to-uint32, zero border

    // INDEX_0 to INDEX_1 is unhilited, INDEX_1 to
    // INDEX_2 is hilited, and INDEX_2 to INDEX_3 is
    // unhilited; each range may be empty
    const StrSize INDEX_1 =
      StringIndexOf( 0, std::max( low_cursor_pos, control.FirstCharShown() ), control.GetLineData() );
    ...
    text_x_pos += fontRenderer->RenderText( Point( text_x_pos, text_y_pos ),
                                            control.GetText().substr( Value( INDEX_0 ), Value( INDEX_1 - INDEX_0 ) ),
                                            text_color_to_use );

    text_x_pos += fontRenderer->RenderText( Point( text_x_pos, text_y_pos ),
                                            control.GetText().substr( Value( INDEX_1 ), Value( INDEX_2 - INDEX_1 ) ),
                                            text_color_to_use );
    text_x_pos += fontRenderer->RenderText( Point( text_x_pos, text_y_pos ),
                                            control.GetText().substr( Value( INDEX_2 ), Value( INDEX_END - INDEX_2 ) ),
                                            text_color_to_use );
  }
  else // no selected text
  {
    fontRenderer->RenderText( Point( client_ul.x, text_y_pos ),
                              control.GetText().substr( Value( INDEX_0 ), Value( INDEX_END - INDEX_0 ) ),
                              text_color_to_use );
...
void WidgetRenderer::RenderListBox( const ListBox& control, int32 depth )
{
  // draw beveled rectangle around client area
  Point ul = control.GetUpperLeft(), lr = control.GetLowerRight();
  Point cl_ul = control.GetClientUpperLeft(), cl_lr = control.GetClientLowerRight();
  Colour color_to_use = control.IsDisabled() ? GetDisabledColour( control.GetColour() ) : control.GetColour();
  Colour int_color_to_use =
    control.IsDisabled() ? GetDisabledColour( control.InteriorColour() ) : control.InteriorColour();
  Colour hilite_color_to_use =
  ...
  for ( ListBox::SelectionSet::iterator sel_it = control.Selections().begin(); sel_it != control.Selections().end();
        ++sel_it )
  {
         ...
         LessThanEqual( curr_sel, last_visible_row, control.end() ) )
    {
      ...
      FlatRectangle( Point( cl_ul.x, cl_ul.y + top ), Point( cl_lr.x, cl_ul.y + bottom ), hilite_color_to_use, CLR_ZERO,
                     static_cast<uint32>( 0 ) ); // int-to-uint32, zero border
      prev_sel = curr_sel;
    }
  }
...
void WidgetRenderer::RenderSlider( const Slider& control, int32 depth )
{
