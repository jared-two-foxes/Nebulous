...
Rect GetBounds( Widget* widget ) { return Rect( widget->GetLowerRight(), widget->GetUpperLeft() ); }

LayoutAssistant::LayoutAssistant( int w, int h )
  : m_defaultCreationWidth( w ), m_defaultCreationHeight( h ), m_resolution( Point( 640, 960 ) )
{
}

void LayoutAssistant::SetResolution( const Point& resolution ) { m_resolution = resolution; }
...
Real LayoutAssistant::GetAspectRatio() const { return ( ( (Real)m_resolution.x ) / m_resolution.y ); }

void LayoutAssistant::Adjust( Widget* widget, const Rect& old_bounds, const Rect& new_bounds )
{
  NE_ASSERT( widget, "" );
  ...
  Point lowerRight( static_cast<int>( x_pos + width ),
                    static_cast<int>( y_pos + height ) ); // float-to-int, values are pixel-aligned
  widget->SizeMove( upperLeft, lowerRight );
}

void LayoutAssistant::Scale( Widget* widget, Real x_scale, Real y_scale )
{
  ...
  widget->SizeMove( position,
                    position + Point( static_cast<int>( width ),
                                      static_cast<int>( height ) ) ); // float-to-int, values are pixel-aligned
  ...
  std::for_each( children.begin(), children.end(),
                 [&]( Widget* child )
                 {
                   ...
                   Point scaled_position(
                     static_cast<int>( relative_position.x * x_scale ),
                     static_cast<int>( relative_position.y * y_scale ) ); // float-to-int, values are pixel-aligned
                   ...
                   child->SizeMove(
                     scaled_position,
                     scaled_position + Point( static_cast<int>( width ),
                                              static_cast<int>( height ) ) ); // float-to-int, values are pixel-aligned
                 } );
}
