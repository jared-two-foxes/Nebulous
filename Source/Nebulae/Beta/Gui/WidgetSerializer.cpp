...
WidgetSerializer::WidgetSerializer( GuiManagerPtr& gui, const RenderSystemPtr& renderDevice,
                                    const SpriteAtlasManagerPtr& atlasManager )
  : m_gui( gui ), m_renderDevice( renderDevice ), m_atlasManager( atlasManager ), m_rootDeviceSize( 640, 960 )
{
}

void WidgetSerializer::SetRootDimensions( const Nebulae::Point& point ) { m_rootDeviceSize = point; }

Widget* WidgetSerializer::ReadIn( File& stream )
///
...
Widget* WidgetSerializer::LoadElement( const Json::Value& elementData, std::shared_ptr<Nebulae::GuiManager> gui )
///
...
///
{
  ...
  {
    Nebulae::ButtonControl* button =
      factory->CreateButton( static_cast<int>( std::floor( position.x ) ), static_cast<int>( std::floor( position.y ) ),
                             width, height, text, font, CLR_WHITE );
  ...
  {
    element = factory->CreateTextControl( static_cast<int>( std::floor( position.x ) ),
                                          static_cast<int>( std::floor( position.y ) ), width, height, text, font,
                                          CLR_BLACK, text_format );
  }
  else if ( strcmp( "edit", type ) == 0 )
  {
    element = factory->CreateEditControl( static_cast<int32>( std::floor( position.x ) ),
                                          static_cast<int32>( std::floor( position.y ) ), width, text, font, CLR_GRAY,
                                          CLR_WHITE, CLR_BLACK );
  }
  else if ( strcmp( "image", type ) == 0 )
  {
    element = factory->CreateStaticGraphic( static_cast<int>( std::floor( position.x ) ),
                                            static_cast<int>( std::floor( position.y ) ), width, height, image );
  }
  else if ( strcmp( "slider", type ) == 0 )
  {
    element = factory->CreateSlider( static_cast<int>( std::floor( position.x ) ),
                                     static_cast<int>( std::floor( position.y ) ), width, height, 0.0f, 1.0f, height );
  }
  else if ( strcmp( "panel", type ) == 0 )
  {
    element = new Nebulae::Control( *factory, static_cast<int>( std::floor( position.x ) ),
                                    static_cast<int>( std::floor( position.y ) ), width, height, 0 );
  }
  else
  {
