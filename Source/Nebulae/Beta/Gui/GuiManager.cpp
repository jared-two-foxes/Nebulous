...
bool GUIImpl::HandlePress( GuiManager* mgr, uint32 mouseButton, const Point& pos, uint64 currTicks )
{
  ...
  m_dragWidgets[mouseButton] =
    m_currWidgetUnderCursor; // track this window as the one being dragged by this mouse button
  ...
  if ( m_dragWidgets[mouseButton] )
  {
    m_widgetRegion = m_dragWidgets[mouseButton]->GetWidgetRegion(
      pos ); // and determine whether a resize-region of it is being dragged
    if ( m_widgetRegion % 3 == 0 )
    { // left regions
...
bool GUIImpl::HandleDrag( GuiManager* mgr, uint32 mouseButton, const Point& pos, uint64 currTicks )
{
  if ( m_widgetRegion == WR_MIDDLE || m_widgetRegion == WR_NONE )
  { // send drag message to window or initiate drag-and-drop
    ...
    {
      if ( !m_dragWidgets[mouseButton]->IsDragable() && m_dragWidgets[mouseButton]->GetDragDropDataType() != "" &&
           mouseButton == 0 )
      {
    ...
    if ( ( m_currDragWidgetDragged && m_dragWidgets[mouseButton]->GetDragDropDataType() != "" && mouseButton == 0 ) ||
         !m_dragDropWidgets.empty() )
    {
      ...
      if ( m_currWidgetUnderCursor && m_prevWidgetUnderCursor == m_currWidgetUnderCursor )
      {
        if ( m_currDragDropHereWidget )
        {
          ...
          m_currWidgetUnderCursor->HandleEvent(
            WidgetEvent( WidgetEvent::DragDropHere, pos, dragDropWidgetsToUse, m_modKeys ) );
        ...
        {
          m_currWidgetUnderCursor->HandleEvent(
            WidgetEvent( WidgetEvent::DragDropEnter, pos, dragDropWidgetsToUse, m_modKeys ) );
          m_currWidgetUnderCursor->DropsAcceptable( m_dragDropWidgetsAcceptable.begin(),
                                                    m_dragDropWidgetsAcceptable.end(), pos );
  ...
  else if ( m_dragWidgets[mouseButton]->IsResizable() )
  { // send appropriate resize message to window
    ...
    {
    case WR_TOPLEFT:
      m_dragWidgets[mouseButton]->SizeMove( offsetPos, m_dragWidgets[mouseButton]->GetRelativeLowerRight() );
      break;
    case WR_TOP:
      m_dragWidgets[mouseButton]->SizeMove( Point( m_dragWidgets[mouseButton]->GetRelativeUpperLeft().x, offsetPos.y ),
    ...
    case WR_TOPRIGHT:
      ...
      break;
    case WR_MIDLEFT:
      m_dragWidgets[mouseButton]->SizeMove( Point( offsetPos.x, m_dragWidgets[mouseButton]->GetRelativeUpperLeft().y ),
                                            m_dragWidgets[mouseButton]->GetRelativeLowerRight() );
      break;
    case WR_MIDRIGHT:
      m_dragWidgets[mouseButton]->SizeMove(
    ...
    case WR_BOTTOM:
      ...
      break;
    case WR_BOTTOMRIGHT:
      m_dragWidgets[mouseButton]->SizeMove( m_dragWidgets[mouseButton]->GetRelativeUpperLeft(), offsetPos );
      break;
    default:
      break;
    }
...
bool GUIImpl::HandleRelease( GuiManager* mgr, uint32 mouseButton, const Point& pos, uint64 currTicks )
{
  m_lastButtonDownRepeatTime = 0;
  m_prevWidgetDragPosition = Point( 0, 0 );
  m_prevWidgetUnderCursorTime = currTicks;
  ...
  bool inDragDrop = !m_dragDropWidgets.empty() || m_currDragWidgetDragged && clickWidget &&
...
Widget* GuiManager::GetFocusWidget() const
{
  return m_impl->m_modalWidgets.empty() ? m_impl->m_focusWidget : m_impl->m_modalWidgets.back().second;
}


Widget* GuiManager::GetWindowUnder( const Point& pt ) const { return m_impl->m_zlist.Pick( pt, GetModalWindow() ); }


uint64 GuiManager::GetDeltaT() const { return m_impl->m_delta_t; }


uint64 GuiManager::GetTicks() const
{
...
uint64 GuiManager::GetButtonDownRepeatDelay() const { return m_impl->m_buttonDownRepeatDelay; }


uint64 GuiManager::GetButtonDownRepeatInterval() const { return m_impl->m_buttonDownRepeatInterval; }


uint64 GuiManager::GetDoubleClickInterval() const { return m_impl->m_doubleClickInterval; }


uint64 GuiManager::GetMinDragTime() const { return m_impl->m_minDragTime; }


uint64 GuiManager::GetMinDragDistance() const { return m_impl->m_minDragDistance; }


bool GuiManager::IsDragDropWidget( const Widget* widget ) const
{
  return m_impl->m_dragDropWidgets.find( const_cast<Widget*>( widget ) ) != m_impl->m_dragDropWidgets.end();
}


bool GuiManager::IsAcceptedDragDropWidget( const Widget* widget ) const
{
...
const std::shared_ptr<WidgetFactory>& GuiManager::GetWidgetFactory() const { return m_impl->m_widgetFactory; }


Widget* GuiManager::Find( const char* name ) const { return m_impl->m_zlist.Find( name ); }


const std::list<Widget*>& GuiManager::GetRegisteredWidgets() const { return m_impl->m_zlist; }


bool GuiManager::HandleEvents( EventType eventType, KeyCode key, uint32 key_code_point, Flags<ModKey> modKeys,
                               const Point& pos, const Point& rel )
///
...
///
{
  ...
  case TOUCHDRAG:
  {
    ...
    else if ( m_impl->m_currWidgetUnderCursor && m_impl->m_prevWidgetUnderCursor == m_impl->m_currWidgetUnderCursor )
    { // if !s_impl->m_drag_wnds[0] and we're moving over the same (valid) object we were during the last iteration
...
void GuiManager::SetFocusWidget( Widget* widget )
{
...
void GuiManager::Register( Widget* widget )
{
  if ( widget )
  {
...
void GuiManager::RegisterModal( Widget* widget )
{
...
void GuiManager::Remove( Widget* widget )
{
  if ( widget )
  {
    ...
    { // if it's not a modal window, remove it from the z-order
...
void GuiManager::WidgetDying( Widget* widget )
{
  if ( widget )
  {
    Remove( widget );
...
void GuiManager::MoveUp( Widget* widget )
{
  if ( widget )
  {
...
void GuiManager::MoveDown( Widget* widget )
{
  if ( widget )
  {
...
void GuiManager::RegisterDragDropWidget( Widget* widget, const Point& offset, Widget* originatingWidget )
{
  NE_ASSERT( widget, "" );
...
void GuiManager::SetDoubleClickInterval( uint32 interval ) { m_impl->m_doubleClickInterval = interval; }


void GuiManager::SetMinDragTime( uint32 time ) { m_impl->m_minDragTime = time; }


void GuiManager::SetMinDragDistance( uint32 distance ) { m_impl->m_minDragDistance = distance; }


void GuiManager::SetWidgetFactory( const std::shared_ptr<WidgetFactory>& factory )
{
...
std::shared_ptr<Font> GuiManager::GetFont( const std::string& filename, unsigned int pts )
{
...
Widget* GuiManager::CheckedGetWindowUnder( const Point& pt, Flags<ModKey> mod_keys )
{
  ...
  if ( widget != m_impl->m_currWidgetUnderCursor )
  {
    ...
    if ( widget )
    {
      if ( unregisteredDragDrop )
      {
        ...
        m_impl->m_dragDropWidgetsAcceptable[draggedWidget] = false;
        widget->DropsAcceptable( m_impl->m_dragDropWidgetsAcceptable.begin(), m_impl->m_dragDropWidgetsAcceptable.end(),
                                 pt );
...
bool Nebulae::MatchesOrContains( const Widget* lwidget, const Widget* rwidget )
{
  if ( rwidget )
  {
