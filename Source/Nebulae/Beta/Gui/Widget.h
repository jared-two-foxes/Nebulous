...
protected:
  ...
  std::vector<Widget*> m_filters; ///< The Wnds that are filtering this Wnd's events. These are in reverse order: top of
                                  ///< the stack is back().
...
protected:
  ...
  /** Respond to the cursor entering the Wnd's coords while dragging
      ...
      offsets from \a pt. */
  virtual void DragDropEnter( const Point& pt, const std::map<Widget*, Point>& dragDropWidgets, Flags<ModKey> modKeys );
  ...
  /** Respond to cursor moving about within the Wnd, or to cursor lingering
      ...
      offsets from \a pt. */
  virtual void DragDropHere( const Point& pt, const std::map<Widget*, Point>& dragDropWidget, Flags<ModKey> modKeys );
