...
EditControl::EditControl( const WidgetFactory& factory, int x, int y, int w, const std::string& str,
                          const std::shared_ptr<Font>& font, Colour color, Colour text_color, Colour interior,
                          uint32 flags )
  : TextControl( factory, x, y, w, HeightFromFont( font, PIXEL_MARGIN ), str, font, text_color,
                 FORMAT_LEFT | FORMAT_IGNORETAGS, flags ),
...
Point EditControl::MinUsableSize() const
{
  return Point( 4 * PIXEL_MARGIN, HeightFromFont( GetFont(), PIXEL_MARGIN ) );
}

Point EditControl::ClientUpperLeft() const { return GetUpperLeft() + Point( PIXEL_MARGIN, PIXEL_MARGIN ); }

Point EditControl::ClientLowerRight() const { return GetLowerRight() - Point( PIXEL_MARGIN, PIXEL_MARGIN ); }

const std::pair<CPSize, CPSize>& EditControl::CursorPosn() const { return m_cursor_pos; }

Colour EditControl::InteriorColour() const { return m_int_color; }

Colour EditControl::HiliteColour() const { return m_hilite_color; }

Colour EditControl::SelectedTextColour() const { return m_sel_text_color; }

void EditControl::SetColour( Colour c ) { Control::SetColour( c ); }

void EditControl::SetInteriorColour( Colour c ) { m_int_color = c; }

void EditControl::SetHiliteColour( Colour c ) { m_hilite_color = c; }

void EditControl::SetSelectedTextColour( Colour c ) { m_sel_text_color = c; }
...
void EditControl::SelectRange( CPSize from, CPSize to )
{
  if ( from < to )
  {
...
bool EditControl::MultiSelected() const { return m_cursor_pos.first != m_cursor_pos.second; }

CPSize EditControl::FirstCharShown() const { return m_first_char_shown; }

bool EditControl::RecentlyEdited() const { return m_recently_edited; }

CPSize EditControl::CharIndexOf( int x ) const
{
  CPSize retval;
  ...
  {
    int curr_extent;
    if ( x + first_char_offset <= ( curr_extent = GetLineData()[0]
                                                    .char_data[Value( retval )]
                                                    .extent ) ) // the point falls within the character at index retval
    {
      int prev_extent = retval ? GetLineData()[0].char_data[Value( retval - 1 )].extent : 0;
      int half_way = ( prev_extent + curr_extent ) / 2;
      if ( half_way <= x + first_char_offset ) // if the point is more than halfway across the character, put the cursor
                                               // *after* the character
      {
        ++retval;
      }
...
int EditControl::FirstCharOffset() const
{
  return ( m_first_char_shown ? GetLineData()[0].char_data[Value( m_first_char_shown - 1 )].extent : 0 );
}

int EditControl::ScreenPosOfChar( CPSize idx ) const
{
  int first_char_offset = FirstCharOffset();
  return GetUpperLeft().x + PIXEL_MARGIN +
         ( ( idx ? GetLineData()[0].char_data[Value( idx - 1 )].extent : 0 ) - first_char_offset );
}

CPSize EditControl::LastVisibleChar() const
{
  int first_char_offset = FirstCharOffset();
  CPSize retval = m_first_char_shown;
  for ( ; retval < Length(); ++retval )
  {
    if ( GetSize().x - 2 * PIXEL_MARGIN <=
         ( retval ? GetLineData()[0].char_data[Value( retval - 1 )].extent : 0 ) - first_char_offset )
    {
      break;
    }
  }
...
void EditControl::TouchDown( const Point& pt, Flags<ModKey> mod_keys )
{
  if ( !IsDisabled() )
  {
...
void EditControl::TouchDrag( const Point& pt, const Point& move, Flags<ModKey> mod_keys )
{
  if ( !IsDisabled() )
  {
    ...
    // when a single-click drag occurs, move m_cursor_pos.second to where the mouse is, which selects a range of
    // characters
  ...
  if ( !IsDisabled() )
  {
    ...
    case VKC_LEFT:
      ...
      else if ( 0 < m_cursor_pos.second )
      {
        ...
        while ( 0 < m_cursor_pos.second &&
                extent == GetLineData()[0].char_data[Value( m_cursor_pos.second - 1 )].extent )
          --m_cursor_pos.second;
        if ( !shift_down )
    ...
    case VKC_RIGHT:
      ...
      {
        int32 extent = GetLineData()[0].char_data[Value( m_cursor_pos.second )].extent;
        while ( m_cursor_pos.second < Length() &&
                extent == GetLineData()[0].char_data[Value( m_cursor_pos.second )].extent )
          ++m_cursor_pos.second;
        if ( !shift_down )
          m_cursor_pos.first = m_cursor_pos.second;
      }
...
void EditControl::GainingFocus() { m_recently_edited = false; }
...
void EditControl::AdjustView()
{
  ...
  int first_char_offset = FirstCharOffset();
  if ( m_cursor_pos.second < m_first_char_shown )
  { // if the caret is at a place left of the current visible area
    if ( m_first_char_shown - m_cursor_pos.second <
         5 ) // if the caret is less than five characters before m_first_char_shown
      m_first_char_shown =
        ( 0 <= m_first_char_shown - 5 ) ? m_first_char_shown - 5 : CP0; // try to move the caret by five characters
      ...
      m_first_char_shown = m_cursor_pos.second;
            ...
            text_space <=
              ( m_cursor_pos.second ? GetLineData()[0].char_data[Value( m_cursor_pos.second - 1 )].extent : 0 ) -
                first_char_offset )
  { // if the caret is moving to a place right of the current visible area
    ...
    if ( last_idx_to_use ==
         Length() - 1 ) // if the caret is at the very end of the string, add the length of some spaces
      pixels_to_move += static_cast<int>( Value( m_cursor_pos.second + 5 - Length() - 1 ) ) * GetFont()->SpaceWidth();
    CPSize move_to = m_first_char_shown;
    while ( move_to < char_data.size() && char_data[Value( move_to )].extent - first_char_offset < pixels_to_move )
    {
      ++move_to;
    }
...
// Free Functions
////////////////////////////////////////////////////////////
void Nebulae::GetTranslatedCodePoint( KeyCode key, uint32 key_code_point, Flags<ModKey> mod_keys,
                                      std::string& translated_code_point )
{
