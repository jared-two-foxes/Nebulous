...
{
  ListSignalEcho( const ListBox& lb, const std::string& name ) : m_LB( lb ), m_name( name ) {}
  void operator()() { std::cerr << "GG SIGNAL : " << m_name << "()\n"; }
  void operator()( const ListBox::SelectionSet& sels )
  {
    std::cerr << "GG SIGNAL : " << m_name << "(sels=[ ";
    for ( ListBox::SelectionSet::const_iterator it = sels.begin(); it != sels.end(); ++it )
    {
  ...
  {
    std::cerr << "GG SIGNAL : " << m_name << "(row=" << RowIndex( it ) << /*" pt=" << pt <<*/ ")\n";
  }
  std::size_t RowIndex( ListBox::const_iterator it ) { return std::distance( m_LB.begin(), it ); }
  const ListBox& m_LB;
  std::string m_name;
};
...
{
public:
  RowSorter( const boost::function<bool( const ListBox::Row&, const ListBox::Row&, std::size_t )>& cmp, std::size_t col,
             bool invert )
...
{
  return lhs == rhs || ListBox::RowPtrIteratorLess<std::list<ListBox::Row*>>::LessThan( lhs, rhs, end );
}
...
ListBox::Row* SafeDeref( const ListBox::iterator& it, const ListBox::iterator& end ) { return it == end ? 0 : *it; }

struct ScopedSet
{
  ScopedSet( ListBox::iterator*& var, ListBox::iterator* value ) : m_var( var = value ) {}
  ~ScopedSet() { m_var = 0; }
...
////////////////////////////////////////////////
ListBox::Row::DeferAdjustLayout::DeferAdjustLayout( Row* row ) : m_row( row ) { m_row->m_ignore_adjust_layout = true; }
...
ListBox::Row::Row( const WidgetFactory& factory, int w, int h, const std::string& drag_drop_data_type, Alignment align,
                   unsigned int margin )
  : Control( factory, 0, 0, w, h ), m_row_alignment( align ), m_margin( margin ), m_ignore_adjust_layout( false )
{
...
std::size_t ListBox::Row::size() const { return m_cells.size(); }

bool ListBox::Row::empty() const { return m_cells.empty(); }

Control* ListBox::Row::operator[]( std::size_t n ) const { return m_cells[n]; }

Control* ListBox::Row::at( std::size_t n ) const { return m_cells.at( n ); }

Alignment ListBox::Row::RowAlignment() const { return m_row_alignment; }

Alignment ListBox::Row::ColAlignment( std::size_t n ) const { return m_col_alignments[n]; }

int ListBox::Row::ColWidth( std::size_t n ) const { return m_col_widths[n]; }

unsigned int ListBox::Row::Margin() const { return m_margin; }
...
void ListBox::Row::push_back( const std::string& str, const std::shared_ptr<Font>& font, Colour color )
{
...
void ListBox::Row::push_back( const Texture& st ) { push_back( CreateControl( st ) ); }
...
ListBox::~ListBox() { delete m_header_row; }
...
void ListBox::DropsAcceptable( DropsAcceptableIter first, DropsAcceptableIter last, const Point& pt ) const
{
  for ( std::map<const Widget*, bool>::iterator it = first; it != last; ++it )
  {
    ...
    if ( row && ( m_allowed_drop_types.find( "" ) != m_allowed_drop_types.end() ||
...
{
  return Nebulae::Point( int( 5 * SCROLL_WIDTH + 2 * BORDER_THICK ), int( 5 * SCROLL_WIDTH + 2 * BORDER_THICK ) );
}

Nebulae::Point ListBox::GetClientUpperLeft() const
{
  return GetUpperLeft() +
         Nebulae::Point( int( BORDER_THICK ),
                         static_cast<int>( BORDER_THICK ) + ( m_header_row->empty() ? 0 : m_header_row->GetHeight() ) );
}

Nebulae::Point ListBox::GetClientLowerRight() const
{
  return GetLowerRight() - Nebulae::Point( static_cast<int>( BORDER_THICK ) + RightMargin(),
                                           static_cast<int>( BORDER_THICK ) + BottomMargin() );
}

bool ListBox::Empty() const { return m_rows.empty(); }

ListBox::const_iterator ListBox::begin() const { return m_rows.begin(); }

ListBox::const_iterator ListBox::end() const { return m_rows.end(); }

ListBox::const_reverse_iterator ListBox::rbegin() const { return m_rows.rbegin(); }

ListBox::const_reverse_iterator ListBox::rend() const { return m_rows.rend(); }
...
ListBox::iterator ListBox::Caret() const { return m_caret; }

const ListBox::SelectionSet& ListBox::Selections() const { return m_selections; }

bool ListBox::Selected( iterator it ) const { return m_selections.find( it ) != m_selections.end(); }

Colour ListBox::InteriorColour() const { return m_int_color; }

Colour ListBox::HiliteColour() const { return m_hilite_color; }

uint32 ListBox::Style() const { return m_style; }

const ListBox::Row& ListBox::ColHeaders() const { return *m_header_row; }

ListBox::iterator ListBox::FirstRowShown() const { return m_first_row_shown; }

std::size_t ListBox::FirstColShown() const { return m_first_col_shown; }
...
std::size_t ListBox::NumRows() const { return m_rows.size(); }

std::size_t ListBox::NumCols() const { return m_col_widths.size(); }

bool ListBox::KeepColWidths() const { return m_keep_col_widths; }

std::size_t ListBox::SortCol() const { return ( m_sort_col < m_col_widths.size() ? m_sort_col : 0 ); }

int ListBox::ColWidth( std::size_t n ) const { return m_col_widths[n]; }

Alignment ListBox::ColAlignment( std::size_t n ) const { return m_col_alignments[n]; }

Alignment ListBox::RowAlignment( iterator it ) const { return ( *it )->RowAlignment(); }

const std::set<std::string>& ListBox::AllowedDropTypes() const { return m_allowed_drop_types; }

bool ListBox::AutoScrollDuringDragDrops() const { return m_auto_scroll_during_drag_drops; }

unsigned int ListBox::AutoScrollMargin() const { return m_auto_scroll_margin; }
...
void ListBox::SetDisabled( bool b ) { Control::SetDisabled( b ); }

void ListBox::SetColour( const Colour& c ) { Control::SetColour( c ); }

ListBox::iterator ListBox::Insert( Row* row, iterator it ) { return Insert( row, it, false ); }

ListBox::iterator ListBox::Insert( Row* row ) { return Insert( row, m_rows.end(), false ); }

ListBox::Row* ListBox::Erase( iterator it ) { return Erase( it, false, false ); }
...
void ListBox::Clear()
{
  ...
  if ( !m_keep_col_widths )
  { // remove column widths and alignments, if needed
    m_col_widths.clear();
...
ListBox::iterator ListBox::begin() { return m_rows.begin(); }

ListBox::iterator ListBox::end() { return m_rows.end(); }

ListBox::reverse_iterator ListBox::rbegin() { return m_rows.rbegin(); }

ListBox::reverse_iterator ListBox::rend() { return m_rows.rend(); }
...
void ListBox::SetSelections( const SelectionSet& s ) { m_selections = s; }

void ListBox::SetCaret( iterator it ) { m_caret = it; }
...
void ListBox::SetInteriorColour( Colour c ) { m_int_color = c; }

void ListBox::SetHiliteColour( Colour c ) { m_hilite_color = c; }
...
void ListBox::RemoveColHeaders() { SetColHeaders( 0 ); }
...
void ListBox::LockColWidths() { m_keep_col_widths = true; }

void ListBox::UnLockColWidths() { m_keep_col_widths = false; }
...
void ListBox::SetRowAlignment( iterator it, Alignment align ) { ( *it )->SetRowAlignment( align ); }

void ListBox::AllowDropType( const std::string& str ) { m_allowed_drop_types.insert( str ); }

void ListBox::DisallowDropType( const std::string& str ) { m_allowed_drop_types.erase( str ); }

void ListBox::AutoScrollDuringDragDrops( bool auto_scroll ) { m_auto_scroll_during_drag_drops = auto_scroll; }

void ListBox::SetAutoScrollMargin( unsigned int margin ) { m_auto_scroll_margin = margin; }

void ListBox::SetAutoScrollInterval( unsigned int /*interval*/ ) { assert( 0 ); }

int ListBox::RightMargin() const { return 0; }

int ListBox::BottomMargin() const { return 0; }

unsigned int ListBox::CellMargin() const { return m_cell_margin; }
...
ListBox::iterator ListBox::OldSelRow() const { return m_old_sel_row; }

ListBox::iterator ListBox::OldRDownRow() const { return m_old_rdown_row; }

ListBox::iterator ListBox::LClickRow() const { return m_lclick_row; }

ListBox::iterator ListBox::RClickRow() const { return m_rclick_row; }

bool ListBox::AutoScrollingUp() const { return m_auto_scrolling_up; }

bool ListBox::AutoScrollingDown() const { return m_auto_scrolling_down; }

bool ListBox::AutoScrollingLeft() const { return m_auto_scrolling_left; }

bool ListBox::AutoScrollingRight() const { return m_auto_scrolling_right; }
...
void ListBox::KeyPress( KeyCode key, uint32 key_code_point, Flags<ModKey> mod_keys )
{
  if ( !IsDisabled() )
  {
    ...
    if ( bring_caret_into_view && key != VKC_SPACE && key != VKC_DELETE && key != VKC_LEFT && key != VKC_RIGHT )
    {
      BringCaretIntoView();
    }
  }
...
void ListBox::DragDropLeave() { ResetAutoScrollVars(); }
...
ListBox::iterator ListBox::Insert( Row* row, iterator it, bool dropped )
{
  ...
  else
  {
    ...
    int y = retval == m_rows.end() ? m_rows.back()->GetRelativeLowerRight().y : ( *retval )->GetRelativeUpperLeft().y;
...
void ListBox::BringCaretIntoView() { BringRowIntoView( m_caret ); }
...
void ListBox::ValidateStyle()
{
  ...
  if ( m_style & LIST_CENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  { // exactly one must be picked; when none or multiples are picked, use LIST_LEFT by default
  ...
  if ( m_style & LIST_TOP )
    ++dup_ct;
  if ( m_style & LIST_BOTTOM )
    ++dup_ct;
  if ( m_style & LIST_VCENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  { // exactly one must be picked; when none or multiples are picked, use LIST_VCENTER by default
...
void ListBox::ClickAtRow( iterator it, Flags<ModKey> mod_keys )
{
  ...
  else
  {
    ...
    { // control key depressed
      ...
      { // just the control key is depressed: toggle the item selected, adjust the caret
    ...
    else if ( mod_keys & MOD_KEY_SHIFT )
    { // shift key depressed
      bool erase = m_selections.find( m_caret ) == m_selections.end();
      if ( !( m_style & LIST_QUICKSEL ) )
        m_selections.clear();
    ...
    { // unless LIST_QUICKSEL is used, this is treated just like LIST_SINGLESEL above
      if ( m_style & LIST_QUICKSEL )
      {
        if ( m_old_sel_row_selected )
          m_selections.erase( it );
