...
Slider::Slider( const WidgetFactory& factory, int x, int y, int w, int h, Real min, Real max, uint32 tabWidth,
                uint32 lineWidth, uint32 flags )
  : Control( factory, x, y, w, h, flags ),
    ...
    m_lineStyle( RAISED ),
    m_tabDragOffset( -1 ),
    m_tab( GetWidgetFactory().CreateSliderTabButton( 0, 0, m_tabWidth, GetHeight(), "", std::shared_ptr<Font>(),
                                                     CLR_WHITE ) ),
    m_draggingTab( false )
{
...
Nebulae::Point Slider::GetMinUsableSize() const
{
...
Real Slider::GetPosn() const { return m_posn; }


std::pair<Real, Real> Slider::GetSliderRange() const { return std::pair<Real, Real>( m_rangeMin, m_rangeMax ); }


Real Slider::GetPageSize() const
{
  return m_pageSize != INVALID_PAGE_SIZE ? m_pageSize : ( m_rangeMax - m_rangeMin ) / 10;
}


uint32 Slider::GetTabWidth() const { return m_tabWidth; }


uint32 Slider::GetLineWidth() const { return m_lineWidth; }


SliderLineStyle Slider::GetLineStyle() const { return m_lineStyle; }

bool Slider::IsTabBeingDragged() const { return ( m_tab->GetState() == ButtonControl::BN_PRESSED ); }


void Slider::SizeMove( const Point& ul, const Point& lr )
{
...
void Slider::SetDisabled( bool b )
{
...
void Slider::SetColour( const Colour& c )
{
  Control::SetColour( c );
  m_tab->SetColour( c );
}
...
void Slider::SizeSlider( Real min, Real max )
{
...
void Slider::SetMax( Real max ) { SizeSlider( m_rangeMin, max ); }


void Slider::SetMin( Real min ) { SizeSlider( min, m_rangeMax ); }


void Slider::SlideTo( Real p ) { SlideToImpl( p, false ); }


ButtonControl* Slider::GetTab() const { return m_tab; }


Real Slider::PtToPosn( const Point& pt ) const
{
...
bool Slider::EventFilter( Widget* w, const WidgetEvent& event )
{
  if ( w == m_tab )
  {
...
void Slider::Clicked( const Point& pt, [[maybe_unused]] Flags<ModKey> mod_keys )
{
  SlideToImpl( m_posn < PtToPosn( pt ) ? m_posn + GetPageSize() : m_posn - GetPageSize(), true );
}


void Slider::KeyPress( KeyCode key, uint32 key_code_point, Flags<ModKey> mod_keys )
{
  if ( !IsDisabled() )
  {
    switch ( key )
...
void Slider::SlideToImpl( Real p, bool signal )
{
  Real oldPosn = m_posn;
