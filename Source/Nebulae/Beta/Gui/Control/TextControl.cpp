...
Nebulae::Point TextControl::GetMinUsableSize() const { return m_text_lr - m_text_ul; }

const std::string& TextControl::GetText() const { return m_text; }

Flags<TextFormat> TextControl::GetTextFormat() const { return m_format; }

Colour TextControl::TextColor() const { return m_text_color; }

bool TextControl::ClipText() const { return m_clip_text; }

bool TextControl::SetMinSize() const { return m_set_min_size; }

bool TextControl::Empty() const { return m_text.empty(); }

CPSize TextControl::Length() const { return m_code_points; }

Nebulae::Point TextControl::TextUpperLeft() const { return GetUpperLeft() + m_text_ul; }

Nebulae::Point TextControl::TextLowerRight() const { return GetUpperLeft() + m_text_lr; }

const std::vector<Font::LineData>& TextControl::GetLineData() const { return m_line_data; }

const std::shared_ptr<Font>& TextControl::GetFont() const { return m_font; }
...
void TextControl::SizeMove( const Nebulae::Point& ul, const Nebulae::Point& lr )
{
  ...
  if ( !m_fit_to_text && ( m_format | FORMAT_WORDBREAK || m_format | FORMAT_LINEWRAP ) )
  {
    ...
    redo_determine_lines = client_width < text_width || ( text_width < client_width && 1u < m_line_data.size() );
  }
...
void TextControl::SetTextColour( Colour colour ) { m_text_color = colour; }
...
void TextControl::ClipText( bool b ) { m_clip_text = b; }
...
void TextControl::Clear() { SetText( "" ); }
...
bool TextControl::FitToText() const { return m_fit_to_text; }

void TextControl::ValidateFormat()
{
  int dup_ct = 0; // duplication count
  if ( m_format & FORMAT_LEFT )
    ++dup_ct;
  if ( m_format & FORMAT_RIGHT )
    ++dup_ct;
  if ( m_format & FORMAT_CENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  ...
  dup_ct = 0;
  if ( m_format & FORMAT_TOP )
    ++dup_ct;
  if ( m_format & FORMAT_BOTTOM )
    ++dup_ct;
  if ( m_format & FORMAT_VCENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  { // exactly one must be picked; when none or multiples are picked, use FORMAT_VCENTER by default
    m_format &= ~( FORMAT_TOP | FORMAT_BOTTOM );
    m_format |= FORMAT_VCENTER;
  }
  if ( ( m_format & FORMAT_WORDBREAK ) && ( m_format & FORMAT_LINEWRAP ) )
...
void TextControl::RecomputeTextBounds()
{
  ...
  {
    m_text_ul.y =
      static_cast<int>( ( GetSize().y - text_sz.y ) / 2.0 ); // double-to-int, truncation intended for pixel alignment
  }
  ...
  {
    m_text_ul.x =
      static_cast<int>( ( GetSize().x - text_sz.x ) / 2.0 ); // double-to-int, truncation intended for pixel alignment
  }
