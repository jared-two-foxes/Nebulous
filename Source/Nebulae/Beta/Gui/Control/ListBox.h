...
enum ListBoxStyle
{
  LIST_NONE,    ///< Default style selected.
  LIST_VCENTER, ///< Cells are aligned with the top of the list box control.
  LIST_TOP,     ///< Cells are aligned with the top of the list box control. This is the default.
  LIST_BOTTOM,  ///< Cells are aligned with the bottom of the list box control.
  LIST_CENTER,  ///< Cells are center-aligned.
  LIST_LEFT,    ///< Cells are left-aligned. This is the default.
  LIST_RIGHT,   ///< Cells are right-aligned.
  LIST_NOSORT,  ///< List items are not sorted. Items are sorted by default.  When used with drag-and-drop, this style
                ///< allows arbitrary rearrangement of list elements by dragging.
  LIST_SORTDESCENDING, ///< Items are sorted based on item text in descending order. Ascending order is the default.
  LIST_NOSEL,          ///< No selection, dragging, or dropping allowed.  This makes the list box effectively read-only.
  LIST_SINGLESEL,      ///< Only one item at a time can be selected. By default, multiple items may be selected.
  LIST_QUICKSEL,   ///< Each click toggles an item without affecting any others; ignored when used with LIST_SINGLESEL.
  LIST_USERDELETE, ///< Allows user to remove selected items by pressing the delete key.
  LIST_BROWSEUPDATES ///< Causes a signal to be emitted whenever the mouse moves over ("browses") a row.
};
...
public:
  /** \brief Sorts iterators to ListBox::Row*s from a container of
      ...
      iterators. */
  template <class Cont> struct RowPtrIteratorLess
  {
  ...
  struct Row : public Control
  {
    ...
    /** \name Structors */               ///@{
    Row( const WidgetFactory& factory ); ///< default ctor
    Row( const WidgetFactory& factory, int w, int h, const std::string& drag_drop_data_type,
         Alignment align = ALIGN_VCENTER, unsigned int margin = 2 ); ///< ctor
    ...
    Control* operator[](
      std::size_t n ) const; ///< returns the Control in the \a nth cell of this Row; not range checked
    Control* at( std::size_t n )
      const; ///< returns the Control in the \a nth cell of this Row \throw std::range_error throws when size() <= \a n

    Alignment RowAlignment() const; ///< returns the vertical alignment of this Row
    Alignment ColAlignment( std::size_t n )
      const; ///< returns the horizontal alignment of the Control in the \a nth cell of this Row; not range checked
    ...
    Control* CreateControl( const std::string& str, const std::shared_ptr<Font>& font, Colour color )
      const; ///< creates a "shrink-fit" TextControl from text, font, and color parameters
    Control* CreateControl(
      const Texture& st ) const; ///< creates a "shrink-fit" StaticGraphic Control from a Texture parameter
    //@}
    ...
    void push_back(
      Control* c ); ///< adds a given Control to the end of the Row; this Control becomes property of the Row
    void push_back(
      const std::string& str, const std::shared_ptr<Font>& font,
      Colour color = CLR_BLACK ); ///< overload of push_back that creates a TextControl and adds it to the Row
    void push_back(
      const std::string& str, const std::string& font_filename, unsigned int pts,
      Colour color = CLR_BLACK ); ///< overload of push_back that creates a TextControl and adds it to the Row
    void push_back(
      const Texture& st );        ///< overload of push_back that creates a StaticGraphic Control and adds it to the Row
    void clear();                 ///< removes and deletes all cells in this Row
    void resize( std::size_t n ); ///< resizes the Row to have \a n cells

    void SetCell( std::size_t n, Control* c ); ///< sets the Control in the \a nth cell of this Row, deleting any
                                               ///< preexisting Control; not range checked
    Control* RemoveCell( std::size_t n ); ///< returns a pointer to the Control in the \a nth cell of this Row, and sets
                                          ///< the contents of the cell to 0; not range checked
    void SetRowAlignment( Alignment align );                ///< sets the vertical alignment of this Row
    void SetColAlignment( std::size_t n, Alignment align ); ///< sets the horizontal alignment of the Control in the \a
                                                            ///< nth cell of this Row; not range checked
    void SetColWidth( std::size_t n, int width ); ///< sets the width of the \a nth cell of this Row; not range checked
    void SetColAlignments( const std::vector<Alignment>& aligns ); ///< sets the horizontal alignment of all the
                                                                   ///< Controls in this Row; not range checked
    void SetColWidths(
      const std::vector<int>& widths ); ///< sets all the widths of the cells of this Row; not range checked
    void SetMargin(
      unsigned int margin ); ///< sets the amount of space left between the contents of adjacent cells, in pixels
                             //@}
  ...
  private:
    ...
    std::vector<Alignment>
      m_col_alignments;            ///< column alignments; each is one of ALIGN_TOP, ALIGN_VCENTER, or ALIGN_BOTTOM
  ...
  typedef boost::signals2::signal<void( const SelectionSet& )>
  ...
  typedef boost::signals2::signal<void( const_iterator )>
    ConstRowSignalType; ///< the signature of const row-change-notification signals
  typedef boost::signals2::signal<void( iterator, const Point& )>
  ...
  typedef ConstRowSignalType
    DropAcceptableSignalType; ///< emitted when a row may be inserted into the list box via drag-and-drop
  typedef RowClickSignalType LeftClickedSignalType; ///< emitted when a row in the listbox is left-clicked; provides the
                                                    ///< row left-clicked and the clicked point
  typedef RowClickSignalType RightClickedSignalType; ///< emitted when a row in the listbox is right-clicked; provides
                                                     ///< the row right-clicked and the clicked point
  typedef RowSignalType DoubleClickedSignalType;     ///< emitted when a row in the listbox is left-double-clicked
  typedef RowSignalType ErasedSignalType;  ///< emitted when a row in the listbox is erased; provides the deleted Row,
                                           ///< and is emitted before the row is removed
  typedef RowSignalType BrowsedSignalType; ///< emitted when a row in the listbox is "browsed" (rolled over) by the
                                           ///< cursor; provides the browsed row
  //@}

  /** \name Structors */ ///@{
  /** basic ctor */
  ListBox( const WidgetFactory& factory, int x, int y, int w, int h, Colour color, Colour interior = CLR_ZERO,
  ...
  /** \name Accessors */ ///@{
  virtual WidgetType GetWidgetType() const { return LISTBOX; }

  virtual void DropsAcceptable( DropsAcceptableIter first, DropsAcceptableIter last, const Point& pt ) const;
  ...
  const Row& GetRow( std::size_t n )
    const; ///< returns a const reference to the row at index \a n; not range-checked.  \note This function is O(n).
  iterator Caret() const; ///< returns the row that has the caret
  const SelectionSet& Selections()
    const;                            ///< returns a const reference to the set row indexes that is currently selected
  bool Selected( iterator it ) const; ///< returns true if row \a it is selected
  Colour InteriorColour() const;      ///< returns the color painted into the client area of the control
  Colour HiliteColour() const;        ///< returns the color behind selected line items

  /** Returns the style flags of the listbox \see ListBoxStyle */
  uint32 Style() const;

  const Row& ColHeaders() const;  ///< returns the row containing the headings for the columns, if any.  If undefined,
                                  ///< the returned heading Row will have size() 0.
  ...
  iterator LastVisibleRow()
    const; ///< returns the last row that could be drawn, taking into account the contents and the size of client area
  std::size_t LastVisibleCol() const; ///< returns the index of the last column that could be drawn, taking into account
                                      ///< the contents and the size of client area
  ...
  Alignment ColAlignment( std::size_t n ) const; ///< returns the alignment of column \a n; must be ALIGN_LEFT,
                                                 ///< ALIGN_CENTER, or ALIGN_RIGHT; not range-checked
  Alignment RowAlignment( iterator it )
    const; ///< returns the alignment of row \a it; must be ALIGN_TOP, ALIGN_VCENTER, or ALIGN_BOTTOM; not range-checked
  ...
  virtual void SizeMove( const Point& ul,
                         const Point& lr ) override; ///< resizes the control, then resizes the scrollbars as needed
  ...
  Row* Erase(
    iterator it ); ///< removes and returns the row that \a it points to from the ListBox, or 0 if no such row exists
  ...
  void SetInteriorColour( Colour c ); ///< sets the color painted into the client area of the control
  void SetHiliteColour( Colour c );   ///< sets the color behind selected line items

  /** sets the style flags for the ListBox to \a s. \see Nebulae::ListBoxStyle */
  void SetStyle( uint32 s );

  void SetColHeaders(
    Row* r );              ///< sets the row used as headings for the columns; this Row becomes property of the ListBox.
  ...
  void SetNumCols(
    std::size_t n ); ///< sets the number of columns in the ListBox to \a n; if no column widths exist before this call,
                     ///< proportional widths are calulated and set, otherwise no column widths are set
  void SetSortCol(
    std::size_t n ); ///< sets the index of the column used to sort rows when sorting is enabled; not range-checked
  ...
  template <class RowType> struct DefaultRowCmp
  {
...
protected:
  ...
  unsigned int CellMargin()
    const; ///< the number of pixels left between the contents of each cell and the cell boundary

  iterator RowUnderPt(
    const Point& pt ) const; ///< returns row under pt, if any; value must be checked (i.e. it may be end())
  ...
  /** \name Mutators */ ///@{
  virtual void KeyPress( KeyCode key, boost::uint32_t key_code_point, Flags<ModKey> mod_keys ) override;
  // virtual void    MouseWheel(const Point& pt, int move, Flags<ModKey> mod_keys) override;
  virtual void DragDropEnter( const Point& pt, const std::map<Widget*, Point>& drag_drop_wnds,
                              Flags<ModKey> mod_keys ) override;
  virtual void DragDropHere( const Point& pt, const std::map<Widget*, Point>& drag_drop_wnds,
                             Flags<ModKey> mod_keys ) override;
  virtual void DragDropLeave() override;

  virtual bool EventFilter( Widget* w, const WidgetEvent& event );

  iterator Insert( Row* row, iterator it, bool dropped ); ///< insertion sorts into list, or inserts into an unsorted
                                                          ///< list before \a it; returns insertion point
  Row* Erase( iterator it, bool removing_duplicate,
              bool signal );  ///< erases the row at index \a idx, handling it as a duplicate removal (such as for
                              ///< drag-and-drops within a single ListBox) if indicated
...
private:
  ...
  void VScrolled( int tab_low, int tab_high, int low,
                  int high ); ///< signals from the vertical scroll bar are caught here
  void HScrolled( int tab_low, int tab_high, int low,
                  int high ); ///< signals from the horizontal scroll bar are caught here
  void ClickAtRow(
    iterator it, Flags<ModKey> modkeys ); ///< handles to a mouse-click or spacebar-click on \a it, modified by \a keys
  void NormalizeRow( Row* row ); ///< adjusts a Row so that it has the same number of cells as other rows, and that each
                                 ///< cell has the correct width and alignment
  iterator FirstRowShownWhenBottomIs(
    iterator bottom_row, int client_height ); ///< Returns the first row shown when the last row shown is \a bottom_row
  std::size_t FirstColShownWhenRightIs(
    std::size_t right_col,
    int client_width ); ///< Returns the index of the first column shown when the last column shown is \a right_col
  ...
  iterator
    m_last_row_browsed; ///< the last row sent out as having been browsed (used to prevent duplicate browse signals)

  iterator m_first_row_shown;    ///< index of row at top of visible area (always begin() for non-empty ListBox with
                                 ///< LIST_NOSCROLL set)
  std::size_t m_first_col_shown; ///< like above, but index of column at left
  std::vector<int> m_col_widths; ///< the width of each of the columns goes here
  ...
  Colour m_int_color;    ///< color painted into the client area of the control
  Colour m_hilite_color; ///< color behind selected line items
  ...
  bool m_clip_cells; ///< if true, the contents of each cell will be clipped to the visible area of that cell (TODO:
                     ///< currently unused)
  ...
  std::set<std::string> m_allowed_drop_types; ///< the line item types allowed for use in this listbox
...
template <class Cont> Nebulae::ListBox::RowPtrIteratorLess<Cont>::RowPtrIteratorLess() : m_container()
{
  NE_ASSERT( m_container, "" );
}

template <class Cont> Nebulae::ListBox::RowPtrIteratorLess<Cont>::RowPtrIteratorLess( Cont* c ) : m_container( c )
{
  NE_ASSERT( m_container, "" );
}

template <class Cont> bool Nebulae::ListBox::RowPtrIteratorLess<Cont>::operator()( Iter lhs, Iter rhs ) const
{
  ...
  BOOST_MPL_ASSERT( (boost::is_same<typename Iter::value_type, Nebulae::ListBox::Row*>));
  return LessThan( lhs, rhs, m_container->end() );
}

template <class Cont> bool Nebulae::ListBox::RowPtrIteratorLess<Cont>::LessThan( Iter lhs, Iter rhs, Iter end )
{
  return lhs == end ? false : ( rhs == end ? true : ( *lhs )->GetUpperLeft().y < ( *rhs )->GetUpperLeft().y );
}

template <class RowType>
bool Nebulae::ListBox::DefaultRowCmp<RowType>::operator()( const Nebulae::ListBox::Row& lhs,
                                                           const Nebulae::ListBox::Row& rhs, std::size_t column ) const
{
