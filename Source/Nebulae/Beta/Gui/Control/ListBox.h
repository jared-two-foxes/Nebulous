...
enum ListBoxStyle {
  ...
  LIST_NOSORT,         ///< List items are not sorted. Items are sorted by default.  When used with drag-and-drop, this style allows arbitrary rearrangement of list elements by dragging.
...
public:
  ...
  struct Row : public Control
  {
    ...
    Row(const WidgetFactory& factory, int w, int h, const std::string& drag_drop_data_type, Alignment align = ALIGN_VCENTER, unsigned int margin = 2); ///< ctor
    ...
    Control*     CreateControl(const std::string& str, const std::shared_ptr<Font>& font, Colour color) const; ///< creates a "shrink-fit" TextControl from text, font, and color parameters
    Control*     CreateControl(const Texture& st) const; ///< creates a "shrink-fit" StaticGraphic Control from a Texture parameter
    //@}
    ...
    void         push_back(const std::string& str, const std::shared_ptr<Font>& font, Colour color = CLR_BLACK); ///< overload of push_back that creates a TextControl and adds it to the Row
    void         push_back(const std::string& str, const std::string& font_filename, unsigned int pts, Colour color = CLR_BLACK); ///< overload of push_back that creates a TextControl and adds it to the Row
    void         push_back(const Texture& st); ///< overload of push_back that creates a StaticGraphic Control and adds it to the Row
...
private:
  ...
  typedef boost::signals2::signal<void (iterator, const Point&)> RowClickSignalType;   ///< the signature of row-click-notification signals
  ...
  /** \name Structors */ ///@{
  /** basic ctor */
  ListBox(const WidgetFactory& factory, int x, int y, int w, int h, Colour color, Colour interior = CLR_ZERO, uint32 flags = INTERACTIVE);
  ...
  virtual void   SizeMove(const Point& ul, const Point& lr) override;  ///< resizes the control, then resizes the scrollbars as needed
...
protected:
  ...
  iterator        RowUnderPt(const Point& pt) const; ///< returns row under pt, if any; value must be checked (i.e. it may be end())
  ...
  virtual void    DragDropEnter(const Point& pt, const std::map<Widget*, Point>& drag_drop_wnds, Flags<ModKey> mod_keys) override;
  virtual void    DragDropHere(const Point& pt, const std::map<Widget*, Point>& drag_drop_wnds, Flags<ModKey> mod_keys) override;
...
private:
  ...
  void            ClickAtRow(iterator it, Flags<ModKey> modkeys); ///< handles to a mouse-click or spacebar-click on \a it, modified by \a keys
...
template <class Cont>
bool Nebulae::ListBox::RowPtrIteratorLess<Cont>::LessThan(Iter lhs, Iter rhs, Iter end)
{
    return lhs == end ?
        false :
        (rhs == end ?
         true : (*lhs)->GetUpperLeft().y < (*rhs)->GetUpperLeft().y);
}

template <class RowType>
bool Nebulae::ListBox::DefaultRowCmp<RowType>::operator()(const Nebulae::ListBox::Row& lhs, const Nebulae::ListBox::Row& rhs, std::size_t column) const
{
