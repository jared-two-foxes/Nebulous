...
public:
  ...
  WidgetEvent( EventType type, const Point& pt, const std::map<Widget*, Point>& dragDropWidgets,
  ...
  uint32 GetKeyCodePoint()
    const; ///< returns the Unicode code point for the key pressed or released in the WidgetEvent, if any.  \note This
           ///< may be zero, even in a KeyPress or KeyRelease event, if Unicode support is unavailable.
  ...
  const std::map<Widget*, Point>& GetDragDropWidgets()
    const; ///< returns the drag-and-drop wnds represented by the WidgetEvent, if any
};
