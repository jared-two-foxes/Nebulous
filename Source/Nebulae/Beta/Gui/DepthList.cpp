...
const int MAX_SPAN =
  1 << 31; // windows should be laid out over no more than 1/2 of possible z-value range (2^32 / 2 = 2^31)
} // namespace
...
void DepthList::Add( Widget* widget )
{
...
bool DepthList::Remove( Widget* widget )
{
  bool retval = false;
  if ( m_contents.find( widget ) != m_contents.end() )
  {
...
bool DepthList::MoveUp( Widget* widget )
{
  bool retval = false;
  iterator it = std::find( begin(), end(), widget );
  if ( it != end() )
  {                                // note that this also implies !empty()..
    ...
    else
    {
      ...
      if ( prev_z - curr_z - 1 >= 3 )
      {
        ...
        // in the middle
        ( *it )->m_zorder = ( *first_non_ontop_it )->m_zorder + ( prev_z - curr_z - 1 ) / 2;
        splice( first_non_ontop_it, *this, it );
      }
      else
      { // make room by bumping up all the on-top windows
...
bool DepthList::MoveDown( Widget* widget )
{
  bool retval = false;
  iterator it = std::find( begin(), end(), widget );
  if ( it != end() )
  {
    ...
    else
    {
      ...
      int curr_z = ( *++first_non_ontop_it )->m_zorder;
      if ( prev_z - curr_z - 1 >= 3 )
      { // if there's at least 3 positions in between..
        ( *it )->m_zorder =
          ( *first_non_ontop_it )->m_zorder + ( prev_z - curr_z - 1 ) / 2; // ..just stick wnd in the middle
        splice( first_non_ontop_it, *this, it );
      }
      else
      { // make room by bumping up all the on-top windows
...
Widget* DepthList::PickWithinWindow( const Point& pt, Widget* widget, const std::set<Widget*>* ignore ) const
{
...
bool DepthList::NeedsRealignment() const
{
  bool retval = false;
  std::size_t sz = size();
  if ( sz > 0 )
  {
    ...
    retval = ( ( range > MAX_SPAN && !max_span_impossible ) || avg_gap > MAX_AVG_GAP_SIZE ||
               avg_gap < MIN_AVG_GAP_SIZE || front_z > MAX_Z || back_z < MIN_Z );
  }
  return retval;
}
...
DepthList::iterator DepthList::GetFirstNonOnTop()
{
