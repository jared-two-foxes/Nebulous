...
const Vector4& Camera::GetLookAt() const { return m_lookAt; }

const Vector4& Camera::GetUp() const { return m_up; }

const Matrix4& Camera::GetViewMatrix() const { return m_view; }

const Matrix4& Camera::GetProjectionMatrix() const { return m_projection; }
...
void Camera::SetLookAt( const Vector4& at )
{
  m_lookAt = at;
  LookAt( m_position, m_lookAt, m_up );
}

void Camera::SetPerspective( const Real fov, const Real aspect, const Real near_, const Real far_ )
{
  m_projection.MakePerspectiveMatrix( fov, near_, far_, aspect );
}

void Camera::SetOrtho( const Real width, const Real height, const Real near_, const Real far_ )
{
...
void Camera::LookAt( const Vector4& vEyePosition, const Vector4& vFocusPosition, const Vector4& vUpDirection )
{
