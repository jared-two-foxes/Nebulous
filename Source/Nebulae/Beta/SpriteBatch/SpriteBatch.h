...
struct GlyphState
{
  ...
  GlyphState() : upperLeft( 0, 0 ), lowerRight( 0, 0 ), colour( CLR_WHITE ), sub_texture( nullptr ), rotation( 0 ) {}

  GlyphState( const Point& ul, const Point& lr )
    : upperLeft( ul ), lowerRight( lr ), colour( CLR_WHITE ), sub_texture( nullptr ), rotation( 0 )
  {
  }
...
public:
  struct QuadVertex
  {
    ...
    QuadVertex( const Point& point, [[maybe_unused]] float depth, const Colour& color,
                const Vector4& textureCoordinates )
      : position( point.x, point.y, -10 /*depth*/, 1.0f ),
        colour( static_cast<float>( color.r ) / 255.f, static_cast<float>( color.g ) / 255.f,
  ...
  /// This needs to be a linked list of VertexList.
  {
    ...
    {
      currentIndex = 0;
      remaining = MAX_VERTICES_PER_BUFFER;
    }
