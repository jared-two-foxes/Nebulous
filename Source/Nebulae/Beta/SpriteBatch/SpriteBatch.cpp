...
SpriteBatch::SpriteBatch( const RenderSystemPtr& renderDevice ) : m_renderDevice( renderDevice ) {}
...
bool SpriteBatch::Init()
{
  ...
  if ( pBuffer == nullptr )
  {
    pBuffer = m_renderDevice->CreateBuffer( "BatchBuffer", HBU_DYNAMIC_WRITE_ONLY,
                                            MAX_VERTICES_PER_BUFFER * sizeof( QuadVertex ), HBB_VERTEX, nullptr );
  }
...
void SpriteBatch::AddQuad( GlyphState* state, std::size_t count )
{
  for ( std::size_t i = 0; i < count; ++i )
  {
    ...
    array.AddVertex( QuadVertex( Point( static_cast<int>( x1 ), static_cast<int>( y1 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx1, ty1, 0, 0 ) ) ); // float-to-int, pixel coordinates
    array.AddVertex( QuadVertex( Point( static_cast<int>( x2 ), static_cast<int>( y1 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx2, ty1, 0, 0 ) ) ); // float-to-int, pixel coordinates
    array.AddVertex( QuadVertex( Point( static_cast<int>( x1 ), static_cast<int>( y2 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx1, ty2, 0, 0 ) ) ); // float-to-int, pixel coordinates
    array.AddVertex( QuadVertex( Point( static_cast<int>( x2 ), static_cast<int>( y1 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx2, ty1, 0, 0 ) ) ); // float-to-int, pixel coordinates
    array.AddVertex( QuadVertex( Point( static_cast<int>( x2 ), static_cast<int>( y2 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx2, ty2, 0, 0 ) ) ); // float-to-int, pixel coordinates
    array.AddVertex( QuadVertex( Point( static_cast<int>( x1 ), static_cast<int>( y2 ) ), state[i].depth,
                                 state[i].colour, Vector4( tx1, ty2, 0, 0 ) ) ); // float-to-int, pixel coordinates
  }
}

void SpriteBatch::AddQuad( const Nebulae::Point& upperLeft, const Nebulae::Point& lowerRight, const Colour& colour,
                           const SubTexture* texture, Real depth, Real rotation )
{
