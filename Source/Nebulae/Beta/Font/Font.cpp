...
void SetJustification( bool& last_line_of_curr_just, Font::LineData& line_data, Alignment orig_just,
                       Alignment prev_just )
{
...
const std::vector<std::pair<boost::uint32_t, boost::uint32_t>> Nebulae::PRINTABLE_ASCII_ALPHA_RANGES =
  boost::assign::list_of( std::pair<boost::uint32_t, boost::uint32_t>( 0x41, 0x5B ) )(
    std::pair<boost::uint32_t, boost::uint32_t>( 0x61, 0x7B ) );

const std::vector<std::pair<boost::uint32_t, boost::uint32_t>> Nebulae::PRINTABLE_ASCII_NONALPHA_RANGES =
  boost::assign::list_of( std::pair<boost::uint32_t, boost::uint32_t>( 0x09, 0x0D ) )(
    std::pair<boost::uint32_t, boost::uint32_t>( 0x20, 0x21 ) )( std::pair<boost::uint32_t, boost::uint32_t>(
    0x30, 0x3A ) )( std::pair<boost::uint32_t, boost::uint32_t>( 0x21, 0x30 ) )(
    std::pair<boost::uint32_t, boost::uint32_t>( 0x3A, 0x41 ) )( std::pair<boost::uint32_t, boost::uint32_t>(
    0x5B, 0x61 ) )( std::pair<boost::uint32_t, boost::uint32_t>( 0x7B, 0x7F ) );
...
CPSize Nebulae::CodePointIndexOf( std::size_t line, CPSize index, const std::vector<Font::LineData>& line_data )
{
  ...
  {
    std::vector<Font::LineData>::const_reverse_iterator it = line_data.rbegin() + ( line_data.size() - 1 - line );
    std::vector<Font::LineData>::const_reverse_iterator end_it = line_data.rend();
    while ( it != end_it )
    {
      if ( !it->char_data.empty() )
...
///////////////////////////////////////
Font::TextElement::TextElement() : whitespace( false ), newline( false ), cached_width( -1 ) {}

Font::TextElement::TextElement( bool ws, bool nl ) : whitespace( ws ), newline( nl ), cached_width( -1 ) {}

Font::TextElement::TextElement( const Font::TextElement& other ) // Copy constructor
  : whitespace( other.whitespace ), newline( other.newline ), cached_width( other.cached_width )
{
  text = other.text;
  widths = other.widths;
}
...
Font::TextElement& Font::TextElement::operator=( const TextElement& ) { return *this; } // Copy assignment operator

Font::TextElement& Font::TextElement::operator=(
  [[maybe_unused]] Font::TextElement&& other ) // Move assignment operator
{
...
StrSize Font::TextElement::StringSize() const { return StrSize( text.size() ); }

CPSize Font::TextElement::CodePointSize() const { return CPSize( widths.size() ); }


///////////////////////////////////////
// class Nebulae::Font::LineData
///////////////////////////////////////
Font::LineData::LineData() : justification( ALIGN_CENTER ) {}

int Font::LineData::Width() const { return char_data.empty() ? 0 : char_data.back().extent; }

bool Font::LineData::Empty() const { return char_data.empty(); }


///////////////////////////////////////
// class Nebulae::Font::LineData::CharData
///////////////////////////////////////
Font::LineData::CharData::CharData() : extent( 0 ), string_index( 0 ) {}

Font::LineData::CharData::CharData( int extent_, StrSize str_index, StrSize str_size, CPSize cp_index )
  : extent( extent_ ), string_index( str_index ), string_size( str_size ), code_point_index( cp_index )
{
}
...
///////////////////////////////////////
Font::Glyph::Glyph() : width( 0 ), height( 0 ), left_bearing( 0 ), vertical_bearing( 0 ), advance( 0 ) {}

Font::Glyph::Glyph( int w, int h, int lb, int vb, int adv )
  : width( w ), height( h ), left_bearing( lb ), vertical_bearing( vb ), advance( adv )
{
}
...
Font::const_iterator Font::begin() const { return m_glyphs.begin(); }

Font::const_iterator Font::end() const { return m_glyphs.end(); }

Font::iterator Font::find( const boost::uint32_t& pattern ) { return m_glyphs.find( pattern ); }

Font::const_iterator Font::find( const boost::uint32_t& pattern ) const { return m_glyphs.find( pattern ); }

const std::string& Font::FontName() const { return m_font_filename; }

unsigned int Font::PointSize() const { return m_pt_sz; }

const std::vector<UnicodeCharset>& Font::UnicodeCharsets() const { return m_charsets; }

int Font::Ascent() const { return m_ascent; }

int Font::Descent() const { return m_descent; }

int Font::Height() const { return m_height; }

int Font::Lineskip() const { return m_lineskip; }

int Font::SpaceWidth() const { return m_space_width; }
...
{
  NE_ASSERT( !text_elements.empty(), "" );
  return DetermineLinesImpl( text, format, box_width, line_data,
...
Nebulae::Point Font::DetermineLinesImpl( const std::string& text, Flags<TextFormat>& format,
                                         [[maybe_unused]] int box_width, std::vector<LineData>& line_data,
                                         std::vector<std::shared_ptr<TextElement>>* text_elements_ptr ) const
{
  ...
  // bool      expand_tabs            = format & FORMAT_LEFT;      // tab expansion only takes place when the lines are
  // left-justified (otherwise, tabs are just spaces)
  bool last_line_of_curr_just =
    false; // is this the last line of the current justification? (for instance when a </right> tag is encountered)
  ...
  for ( std::size_t i = 0, n = text_elements.size(); i < n; ++i )
  {
    ...
    while ( it != end_it )
    {
      ...
      line_data.back().char_data.push_back(
        LineData::CharData( x, original_string_offset + char_index, char_size, code_point_offset ) );
    ...
    {
      line_data.push_back( LineData() );
      SetJustification( last_line_of_curr_just, line_data.back(), orig_just,
...
bool Font::Load( File* is )
{
  ...
  std::vector<std::pair<boost::uint32_t, boost::uint32_t>> range_vec( PRINTABLE_ASCII_NONALPHA_RANGES.begin(),
  ...
  if ( m_charsets.empty() )
  {
    range_vec.insert( range_vec.end(), PRINTABLE_ASCII_ALPHA_RANGES.begin(), PRINTABLE_ASCII_ALPHA_RANGES.end() );
  }
  else
  {
  ...
  for ( std::size_t i = 0; i < range_vec.size(); ++i )
  {
    ...
    for ( boost::uint32_t c = low; c < high; ++c )
    {
      ...
      {
        m_glyphs[c] = Glyph( face.GetGlyphWidth(), face.GetGlyphHeight(), face.GetGlyphLeftBearing(),
                             face.GetGlyphVerticalBearing(), face.GetGlyphAdvance() );
      }
    }
  }
...
void Font::ValidateFormat( Flags<TextFormat>& format ) const
{
  ...
  if ( format & FORMAT_CENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  { // exactly one must be picked; when none or multiples are picked, use FORMAT_LEFT by default
    format &= ~( FORMAT_RIGHT | FORMAT_CENTER );
    format |= FORMAT_LEFT;
  }
  dup_ct = 0;
  if ( format & FORMAT_TOP )
    ++dup_ct;
  ...
  if ( format & FORMAT_VCENTER )
    ++dup_ct;
  if ( dup_ct != 1 )
  { // exactly one must be picked; when none or multiples are picked, use FORMAT_TOP by default
    format &= ~( FORMAT_BOTTOM | FORMAT_VCENTER );
    format |= FORMAT_TOP;
  }
  if ( ( format & FORMAT_WORDBREAK ) && ( format & FORMAT_LINEWRAP ) )
  { // only one of these can be picked; FORMAT_WORDBREAK overrides FORMAT_LINEWRAP
