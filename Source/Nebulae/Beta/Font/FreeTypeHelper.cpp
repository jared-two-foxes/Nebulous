...
struct FTLibraryWrapper
{
  FTLibraryWrapper() : m_library( 0 )
  {
    ...
    { // if no library exists and we can't create one...
...
const double ITALICS_FACTOR =
  1.0 / tan( ( 90 - ITALICS_SLANT_ANGLE ) * 3.1415926 /
             180.0 ); // factor used to shear glyphs ITALICS_SLANT_ANGLE degrees CW from straight up
...
FontFaceProxy::FontFaceProxy() : m_face( 0 ) {}
...
FT_Error FontFaceProxy::GetFace( const char* filename )
{
...
FT_Error FontFaceProxy::GetFace( File& file )
{
  std::size_t length;
...
bool FontFaceProxy::SetCharSize( int size )
{
...
int FontFaceProxy::GetHeight() const { return GetAscent() - GetDecent() + 1; }


int FontFaceProxy::GetAscent() const
{
...
int FontFaceProxy::GetDecent() const
{
  // convert from fixed-point 26.6 format
...
int FontFaceProxy::GetLineSkip() const { return static_cast<int>( m_face->size->metrics.height / 64.0 ); }


double FontFaceProxy::GetUnderlineOffset() const
{
  return std::floor( FT_MulFix( m_face->underline_position, m_face->size->metrics.y_scale ) / 64.0 );
}


double FontFaceProxy::GetUnderlineHeight() const
{
  return std::max<int>(
    1,
    static_cast<int>( std::ceil( FT_MulFix( m_face->underline_thickness, m_face->size->metrics.y_scale ) / 64.0 ) ) );
}


double FontFaceProxy::GetItalicOffset() const { return ( ITALICS_FACTOR * GetHeight() / 2.0 ); }


bool FontFaceProxy::GenerateGlyph( boost::uint32_t ch )
{
  bool retval = true;
...
int FontFaceProxy::GetGlyphTop() const
///
...
int FontFaceProxy::GetGlyphLeftBearing() const
{
  return static_cast<int>( ( std::ceil( m_face->glyph->metrics.horiBearingX / 64.0 ) ) );
}


int FontFaceProxy::GetGlyphVerticalBearing() const
{
  return static_cast<int>( ( std::ceil( m_face->glyph->metrics.horiBearingY / 64.0 ) ) );
}


int FontFaceProxy::GetGlyphAdvance() const
{
  return static_cast<int>( ( std::ceil( m_face->glyph->metrics.horiAdvance / 64.0 ) ) );
}


int FontFaceProxy::GetGlyphWidth() const
{
  return static_cast<int>( ( std::ceil( m_face->glyph->metrics.width / 64.0 ) ) );
}


int FontFaceProxy::GetGlyphHeight() const
{
  return static_cast<int>( ( std::ceil( m_face->glyph->metrics.height / 64.0 ) ) );
}


uint8* FontFaceProxy::GetBitmapBuffer() const { return m_face->glyph->bitmap.buffer; }


int FontFaceProxy::GetBitmapPitch() const { return m_face->glyph->bitmap.pitch; }


int FontFaceProxy::GetBitmapWidth() const { return m_face->glyph->bitmap.width; }


int FontFaceProxy::GetBitmapHeight() const { return m_face->glyph->bitmap.rows; }


void FontFaceProxy::CheckFace( [[maybe_unused]] FT_Error error )
{
