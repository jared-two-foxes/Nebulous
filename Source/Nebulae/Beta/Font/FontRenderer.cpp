...
template <class T> T NextPowerOfTwo( T input )
{
  T value( 1 );
  while ( value < input )
  {
...
{
  TempGlyphData() {}
  TempGlyphData( std::size_t i, const Nebulae::Point& ul_, const Nebulae::Point& lr_ ) : idx( i ), ul( ul_ ), lr( lr_ )
  {
  }
...
FontRenderer::FontRenderer( RenderSystemPtr renderSystem, SpriteBatch* batcher, const std::shared_ptr<Font>& font )
  : OverlayRenderer( renderSystem, batcher ), m_font( font )
{
}


const std::shared_ptr<Font> FontRenderer::GetFont() const { return m_font; }


bool FontRenderer::Init( File* is )
{
  ...
  else
  {
    ...
    {
      range_vec.push_back(
        Pair( m_font->UnicodeCharsets()[i].m_first_char, m_font->UnicodeCharsets()[i].m_last_char ) );
    }
  }
  ...
  for ( std::size_t i = 0; i < range_vec.size(); ++i )
  {
    ...
    for ( boost::uint32_t c = low; c < high; ++c )
    {
      ...
      {
        if ( x + face.GetBitmapWidth() >= BUF_WIDTH )
        { // start a new row of glyph images
          if ( x > max_x )
            max_x = x;
          x = 0;
          y += m_font->Height();
          if ( y + m_font->Height() >= BUF_HEIGHT )
          { // if there's not enough room for another row, create a new buffer
            // cut off bottom portion of buffer just written, if it is possible to do so and maintain power-of-two
            // height
        ...
        temp_glyph_data[c] = TempGlyphData( static_cast<int>( buffer_vec.size() ) - 1, Point( x, y ),
  ...
  for ( std::size_t i = 0; i < buffer_vec.size(); ++i )
  {
    ...
    texture->LoadFromMemory( (unsigned char*)( buffer_vec[i] ), 4 /*GL_UNSIGNED_BYTE*/, 2 /*GL_LUMINANCE_ALPHA*/,
                             buffer_sizes[i].x, buffer_sizes[i].y );
    m_textures.push_back( texture );
    delete[] buffer_vec[i];
  }

  // Create Glyph objects from temp glyph data
  for ( std::map<boost::uint32_t, TempGlyphData>::iterator it = temp_glyph_data.begin(); it != temp_glyph_data.end();
        ++it )
  {
    ...
    m_glyphTextures[it->first] =
      SubTexture( texture, float( it->second.ul.x ) / width, float( it->second.ul.y ) / height,
                  float( it->second.lr.x ) / width, float( it->second.lr.y ) / height );
  }
...
int FontRenderer::RenderGlyph( const Point& pt, char c, RenderState* render_state ) const
{
...
int FontRenderer::RenderGlyph( const Point& pt, boost::uint32_t c, RenderState* render_state ) const
{
...
int FontRenderer::RenderText( const Point& pt_, const std::string& text, const Colour& colour,
...
void FontRenderer::RenderText( const Point& ul, const Point& lr, const std::string& text, Flags<TextFormat>& format,
                               ...
                               RenderState* render_state ) const
{
  ...
  RenderText( ul, lr, text, format, *line_data, *render_state, 0, CP0, line_data->size(),
              CPSize( line_data->back().char_data.size() ) );
}


void FontRenderer::RenderText( const Point& ul, const Point& lr, const std::string& text, Flags<TextFormat>& format,
                               const std::vector<Font::LineData>& line_data, RenderState& render_state,
                               std::size_t begin_line, CPSize begin_char, std::size_t end_line, CPSize end_char ) const
{
  ...
  {
    y_origin = ul.y + static_cast<int>(
                        ( ( lr.y - ul.y ) -
                          ( static_cast<int>( end_line - begin_line - 1 ) * m_font->Lineskip() + m_font->Height() ) ) /
                        2.0f );
  }
...
int FontRenderer::RenderGlyph( const Point& pt, const Font::Glyph& glyph, const SubTexture& subtexture,
