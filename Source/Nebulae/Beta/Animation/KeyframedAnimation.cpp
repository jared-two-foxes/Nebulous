...
void KeyframedAnimation::sampleTracks( Real time, Transform* transformTracksOut ) const
{
  ...
  memcpy( transformTracksOut, &m_Transforms[first * m_iTransformsPerFrame],
          sizeof( Transform ) * m_iTransformsPerFrame );
}

// getNumOriginalFrames
std::size_t KeyframedAnimation::getNumOriginalFrames() const { return ( m_Transforms.size() / m_iTransformsPerFrame ); }
