...
SpriteAtlas::SpriteAtlas( const std::string& strName, RenderSystemPtr renderDevice )
  : m_name( strName ), m_renderDevice( renderDevice )
{
}
...
bool SpriteAtlas::Load( File* is )
{
  ...
  NE_ASSERT( texture != nullptr && texture->GetWidth() == image_width,
             "Image width does not match specified width in atlas desc file." );
  NE_ASSERT( texture != nullptr && texture->GetHeight() == image_height,
             "Image height does not match specified height in atlas desc file." );
...
const std::string& SpriteAtlas::GetName() const { return m_name; }


std::size_t SpriteAtlas::GetModuleCount() const { return m_modules.size(); }


SubTexture* SpriteAtlas::FindModuleSubTexture( const std::string& moduleName ) const
///
...
void SpriteAtlas::ProcessFrame( Json::Value& data, Texture* texture )
{
