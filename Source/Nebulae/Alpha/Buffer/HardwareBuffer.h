...
extern const LockOptions
  HBL_DISCARD; ///< Discards the entire buffer while locking;  This allows optimisation to be performed because
               ///< synchronisation issues are relaxed.  Only allowed on buffers created with the HBU_DYNAMIC flag.
extern const LockOptions
  HBL_READ_ONLY; ///< Lock the buffer for reading only.  Not allowed in buffers which are created with HBU_WRITE_ONLY.
                 ///< Mandatory on static buffers, ie those created without the HBU_DYNAMIC flag.
extern const LockOptions
  HBL_NO_OVERWRITE; ///< As HBL_DISCARD, except the application guarantees not to overwrite any region of the buffer
                    ///< which has already been used in this frame, can allow some optimisations on some API's.
