...
Nebulae::PixelFormat EnginePixelFormatFromPng( int colorType, int bitDepth )
{
...
int GetChannelsFromColorType( int colorType )
{
...
ImageCodecData* PngCodec::Decode( File& is ) const
///
...
///
{
  ...
  png_structp png_ptr = png_create_read_struct( PNG_LIBPNG_VER_STRING, nullptr, nullptr,
                                                nullptr ); // the nulls are error function callbacks,
  if ( !png_ptr )
  {
    return nullptr;
  }
...
ImageCodecData* PngCodec::ConvertFromRawBits( uint8* bits, PixelFormat sourceFormat, std::size_t width,
