...
long DLL_CALLCONV StreamTell( fi_handle handle ) { return file_tell( handle ); }
...
FIBITMAP* ConvertFromRawBitsEx( BYTE* bits, FREE_IMAGE_TYPE type, std::size_t width, std::size_t height,
                                std::size_t pitch, unsigned bpp, unsigned red_mask, unsigned green_mask,
                                unsigned blue_mask, BOOL topdown )
///
/// An extension to the FreeImage_ConvertFromRawBits function which accepts an extra argument which describes the Image
/// data format.  Creates a FIBITMAP object from the specified parameter information.  Currently only tested with
...
Nebulae::ImageCodecData* GenerateImageDataFromBitmap( FIBITMAP* bitmap )
///
...
ImageCodecData* FreeImageCodec::Decode( File& is ) const
///
...
ImageCodecData* FreeImageCodec::ConvertFromRawBits( uint8* bits, PixelFormat sourceFormat, std::size_t width,
                                                    std::size_t height, std::size_t pitch, uint8 bpp,
                                                    PixelFormat returnFormat ) const
///
/// Convert an image from one format to another.  Attempts to interpret the raw image data using the specified
/// parameters and create a FIBITMAP object which is then converted to the specified "out" image format using the
/// FreeImage_ConvertToType function.
///
/// @param bits
...
///
{
  ...
  if ( sourceType == FIT_UNKNOWN )
    return NULL;

  FIBITMAP* bitmap = FreeImage::ConvertFromRawBitsEx( bits, sourceType, width, height, pitch, bpp, FI_RGBA_RED_MASK,
