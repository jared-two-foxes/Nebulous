...
TextureImpl* Texture::GetImpl() const { return m_impl; }


TextureType Texture::GetTextureType() const { return m_impl->GetTextureType(); }


PixelFormat Texture::GetFormat() const { return m_impl->GetFormat(); }


std::size_t Texture::GetWidth() const { return m_impl->GetWidth(); }


std::size_t Texture::GetHeight() const { return m_impl->GetHeight(); }


std::size_t Texture::GetDepth() const { return m_impl->GetDepth(); }


void Texture::ConvertToImage( Image& destImage, bool includeMipMaps ) const
{
  ////@todo implementation needed.
  // std::size_t numMips  = 1; //includeMipMaps? getNumMipmaps() + 1 : 1;
  // std::size_t dataSize = Image::calculateSize( numMips, 1/*GetNumFaces()*/, GetWidth(), GetHeight(), GetDepth(),
  // GetFormat() );
...
void Texture::SetImpl( TextureImpl* impl ) { m_impl = impl; }


bool Texture::LoadImpl_( File* is )
{
  if ( !m_impl || !is )
  {
    return false;
...
bool Texture::UnloadImpl_() { return ( m_impl ? m_impl->Unload() : false ); }


bool Texture::LoadFromMemory( unsigned char* pData, uint8 srcDataType, uint8 channels, int iWidth, int iHeight )
{
