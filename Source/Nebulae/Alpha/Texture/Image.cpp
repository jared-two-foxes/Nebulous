...
Image& Image::loadDynamicImage( uint8* data, std::size_t width, std::size_t height, std::size_t depth,
                                PixelFormat format, bool autoDelete, std::size_t numFaces, std::size_t numMipMaps )
{
  m_width = width;
  m_height = height;
  // m_depth      = depth;
...
Image& Image::convert( PixelFormat conversionFormat )
{
  // ImageCodec*     codec = new FreeImageCodec();
  // std::size_t     pitch = m_width * PixelUtil::GetNumElemBytes(m_format);
  // ImageCodecData* data  = codec->ConvertFromRawBits( m_buffer, m_format, m_width, m_height, pitch, m_pixelSize * 8,
  // conversionFormat ); if( data ) {
  //   m_width      = data->width;
  //   m_height     = data->height;
  //   m_bufSize    = data->size;
  //   m_format     = data->format;
...
uint8* Image::getData() { return m_buffer; }


const uint8* Image::getData() const { return m_buffer; }


std::size_t Image::getSize() const { return m_bufSize; }


std::size_t Image::getWidth() const { return m_width; }


std::size_t Image::getHeight() const { return m_height; }


std::size_t Image::getRowSpan() const { return 0; }


PixelFormat Image::getFormat() const { return m_format; }


uint8 Image::getBPP() const { return m_pixelSize; }


bool Image::hasAlpha() const { return false; }


Colour Image::getColourAt( std::size_t x, std::size_t y ) const
{
...
std::size_t Image::calculateSize( std::size_t mipmaps, std::size_t faces, std::size_t width, std::size_t height,
                                  std::size_t depth, PixelFormat format )
{
  std::size_t size = 0;
