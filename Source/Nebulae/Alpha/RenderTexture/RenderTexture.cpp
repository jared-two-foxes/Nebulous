...
Nebulae::RenderTextureImpl* RenderTexture::GetImpl() const { return m_impl; }

std::size_t RenderTexture::GetWidth() const
{
  NE_ASSERT( m_colour != nullptr, "Attempting to access an invalid RenderTextureImpl." );
  return m_colour->GetWidth();
}

std::size_t RenderTexture::GetHeight() const
{
  NE_ASSERT( m_colour != nullptr, "Attempting to access an invalid RenderTextureImpl." );
  return m_colour->GetHeight();
}

void RenderTexture::SetImpl( Nebulae::RenderTextureImpl* impl )
{
  NE_ASSERT( impl != nullptr, "Attempting to set an invalid RenderTextureImpl." );
  m_impl = impl;
}

bool RenderTexture::BindColourBuffer( int32 index, int32 width, int32 height )
{
...
bool RenderTexture::BindColourTexture( int32 index, Nebulae::Texture* texture )
{
  //@todo handle multiple index binding!
