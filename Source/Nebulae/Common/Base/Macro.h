...
/// marks member functions as being an override of a base class virtual function. uses nonstandard C++ extensions
/// provided by MSVC
...
/// tells the compiler that the return value (RV) of a function is an object that will not be aliased with any other
/// pointers
#define NE_RESTRICT_RV __declspec( restrict )

/// tells the compiler that a function call does not modify or reference visible global state and only modifies the
/// memory pointed to directly by pointer parameters
...
#define NE_UNUSED( ... )                                         \
  {                                                              \
    ...
    ( (void)( __VA_ARGS__ ) ) _Pragma( "clang diagnostic pop" ); \
  }
#else
...
// NE_VA_NUM_ARGS() is a very nifty macro to retrieve the number of arguments handed to a variable-argument macro, caps
// at 10 arguments. The very handy implementation which returns 0 when no arguments are found which is used below can be
// found at
...
#define NE_VA_NUM_ARGS_ZERO_BASED( ... ) \
  NE_VA_NUM_ARGS_HELPER NE_LEFT_PARENTHESIS __VA_ARGS__, NE_VA_NUM_ARGS_REVERSE_SEQUENCE NE_RIGHT_PARENTHESIS
...
#if _MSC_VER >= 1400
#define NE_EXPAND_ARGS( op, ... ) \
  NE_JOIN( NE_EXPAND_ARGS_, NE_VA_NUM_ARGS( __VA_ARGS__ ) ) NE_LEFT_PARENTHESIS op, __VA_ARGS__ NE_RIGHT_PARENTHESIS
#else
...
#define NE_EXPAND_ARGS_7( op, a1, a2, a3, a4, a5, a6, a7 ) \
  op( a1 ) op( a2 ) op( a3 ) op( a4 ) op( a5 ) op( a6 ) op( a7 )
#define NE_EXPAND_ARGS_8( op, a1, a2, a3, a4, a5, a6, a7, a8 ) \
  op( a1 ) op( a2 ) op( a3 ) op( a4 ) op( a5 ) op( a6 ) op( a7 ) op( a8 )
#define NE_EXPAND_ARGS_9( op, a1, a2, a3, a4, a5, a6, a7, a8, a9 ) \
  op( a1 ) op( a2 ) op( a3 ) op( a4 ) op( a5 ) op( a6 ) op( a7 ) op( a8 ) op( a9 )
#define NE_EXPAND_ARGS_10( op, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10 ) \
