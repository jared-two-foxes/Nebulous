...
inline int Value( int i ) { return i; }

inline double Value( double d ) { return d; }

inline std::size_t Value( std::size_t s ) { return s; }

} // namespace Nebulae

#define NE_MEMBER_BOOL_OP_SELF_TYPE( op, rhs_type ) \
  inline bool operator op( rhs_type rhs ) const { return m_value op rhs.m_value; }

#define NE_MEMBER_BOOL_OP_OTHER_TYPE( op, rhs_type ) \
  inline bool operator op( rhs_type rhs ) const { return m_value op Value( rhs ); }
...
#define NE_MEMBER_ASSIGN_OP_OTHER_TYPE( op, self_type, rhs_type )            \
  ...
  {                                                                          \
    m_value = static_cast<self_type::value_type>( m_value op Value( rhs ) ); \
    return *this;                                                            \
  }

#define NE_MEMBER_OP_OTHER_TYPE_DECL( op, self_type, rhs_type ) inline self_type& operator op( rhs_type rhs ) const

#define NE_MEMBER_OP_OTHER_TYPE( op, self_type, rhs_type ) \
  NE_MEMBER_OP_OTHER_TYPE_DECL( op, self_type, rhs_type ) { return self_type( m_value op Value( rhs ) ); }

#define NE_NONMEMBER_OP_SELF_TYPE( op, self_type ) \
  inline self_type operator op( self_type lhs, self_type rhs ) { return lhs op## = rhs; }

#define NE_NONMEMBER_OP_OTHER_TYPE( op, self_type, rhs_type ) \
  inline self_type operator op( self_type lhs, rhs_type rhs ) { return lhs op## = Value( rhs ); }
...
#define NE_NONMEMBER_REVERSED_BOOL_OP_SET( lhs_type, self_type )                    \
  inline bool operator==( lhs_type x, self_type y ) { return y == x; }              \
  inline bool operator!=( lhs_type x, self_type y ) { return y != x; }              \
  inline bool operator<( lhs_type x, self_type y ) { return !( y < x || y == x ); } \
  inline bool operator>( lhs_type x, self_type y ) { return !( y > x || y == x ); } \
  inline bool operator<=( lhs_type x, self_type y ) { return !( y < x ); }          \
  inline bool operator>=( lhs_type x, self_type y ) { return !( y > x ); }

#define NE_NONMEMBER_REVERSED_ARITH_OP_SET( lhs_type, self_type )          \
  inline self_type operator+( lhs_type x, self_type y ) { return y += x; } \
  inline self_type operator*( lhs_type x, self_type y ) { return y *= x; }
...
#define NE_STRONG_SIZE_TYPEDEF( name )                                                                  \
  ...
  public:                                                                                               \
    ...
    NE_MEMBER_OTHER_COMPARATORS( std::size_t );                                                         \
                                                                                                        \
    operator int ConvertibleToBoolDummy::*() const { return m_value ? &ConvertibleToBoolDummy::_ : 0; } \
                                                                                                        \
  ...
  NE_NONMEMBER_REVERSED_ARITH_OP_SET( std::size_t, name );                                              \
                                                                                                        \
  inline std::size_t Value( name x ) { return x.m_value; }                                              \
                                                                                                        \
