...
struct Rect
{
  /** \name Structors */ ///@{
  Rect();                ///< default ctor
  Rect( const Point& pt1,
        const Point& pt2 ); ///< ctor that constructs a Rect from two corners; any two opposing corners will do
  Rect( int x1, int y1, int x2,
        int y2 ); ///< ctor that constructs a Rect from its left, upper, right, and bottom boundaries
  //@}
  ...
  void operator+=( const Point& pt )
  {
    ul += pt;
    lr += pt;
  } ///< shifts the Rect by adding \a pt to each corner
  void operator-=( const Point& pt )
  {
    ul -= pt;
    lr -= pt;
  } ///< shifts the Rect by subtracting \a pt from each corner
  //@}
...
{
  return lhs.x < rhs.x && lhs.y < rhs.y;
} ///< returns true if \a lhs.x and \a lhs.y are both less than the corresponding components of \a rhs
inline bool operator>( const Point& lhs, const Point& rhs )
{
  return lhs.x > rhs.x && lhs.y > rhs.y;
} ///< returns true if \a lhs.x and \a lhs.y are both greater than the corresponding components of \a rhs
inline bool operator<=( const Point& lhs, const Point& rhs )
{
  return lhs.x <= rhs.x && lhs.y <= rhs.y;
} ///< returns true if \a lhs.x and \a lhs.y are both less than or equal to the corresponding components of \a rhs
inline bool operator>=( const Point& lhs, const Point& rhs )
{
  return lhs.x >= rhs.x && lhs.y >= rhs.y;
} ///< returns true if \a lhs.x and \a lhs.y are both greater than or equal to the corresponding components of \a rhs
inline Point operator+( const Point& lhs, const Point& rhs )
{
  return Point( lhs.x + rhs.x, lhs.y + rhs.y );
} ///< returns the vector sum of \a lhs and \a rhs
inline Point operator-( const Point& lhs, const Point& rhs )
{
  return Point( lhs.x - rhs.x, lhs.y - rhs.y );
} ///< returns the vector difference of \a lhs and \a rhs

/** returns true if \a lhs is identical to \a rhs */
inline bool operator==( const Rect& lhs, const Rect& rhs )
{
  return lhs.ul.x == rhs.ul.x && lhs.lr.x == rhs.lr.x && lhs.lr.x == rhs.lr.x && lhs.lr.y == rhs.lr.y;
}

/** returns true if \a lhs differs from \a rhs */
inline bool operator!=( const Rect& lhs, const Rect& rhs ) { return !( lhs == rhs ); }

inline Rect operator+( const Rect& rect, const Point& pt )
{
  return Rect( rect.ul + pt, rect.lr + pt );
} ///< returns \a rect shifted by adding \a pt to each corner
inline Rect operator-( const Rect& rect, const Point& pt )
{
  return Rect( rect.ul - pt, rect.lr - pt );
} ///< returns \a rect shifted by subtracting \a pt from each corner
inline Rect operator+( const Point& pt, const Rect& rect )
{
  return rect + pt;
} ///< returns \a rect shifted by adding \a pt to each corner
inline Rect operator-( const Point& pt, const Rect& rect )
