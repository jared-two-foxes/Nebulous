...
template <class T> struct is_flag_type : std::false_type
{
};
...
#define NE_FLAG_TYPE( name )                                                          \
  class name;                                                                         \
                                                                                      \
  template <> struct is_flag_type<name> : std::true_type                              \
  {                                                                                   \
  };                                                                                  \
                                                                                      \
  class name                                                                          \
  ...
  public:                                                                             \
    ...
    {                                                                                 \
      if ( 1u < detail::OneBits( value ) )                                            \
        throw std::invalid_argument( "Non-bitflag passed to " #name " constructor" ); \
    }                                                                                 \
    bool operator==( name rhs ) const { return m_value == rhs.m_value; }              \
    bool operator!=( name rhs ) const { return m_value != rhs.m_value; }              \
    bool operator<( name rhs ) const { return m_value < rhs.m_value; }                \
                                                                                      \
  private:                                                                            \
    ...
    friend class Flags<name>;                                                         \
                                                                                      ...
                                                                                      \
  template <> FlagSpec<name>& FlagSpec<name>::instance();                             \
                                                                                      \
...
template <class FlagType> class FlagSpec
{
public:
  ...
  {
    for ( typename std::map<FlagType, std::string>::const_iterator it = m_strings.begin(); it != m_strings.end(); ++it )
    {
      if ( it->second == str )
        return it->first;
    }
    // NE_LOG( "Could not find flag corresponding to unknown string" );
    return FlagType( 0 );
  }
  //@}
...
template <class FlagType> class Flags;

template <class FlagType> std::ostream& operator<<( std::ostream& os, Flags<FlagType> flags );
...
public:
  ...
  Flags( FlagType flag ) : m_flags( flag.m_value )
  {
    // NE_ASSERT( FlagSpec<FlagType>::instance().contains(flag), "Invalid flag with value %s",
    // boost::lexical_cast<std::string>(flag.m_value) )();
  }
  //@}
  ...
  operator int ConvertibleToBoolDummy::*() const { return m_flags ? &ConvertibleToBoolDummy::_ : 0; }
  /** Returns true iff *this contains the same flags as \a rhs. */
  bool operator==( Flags<FlagType> rhs ) const { return m_flags == rhs.m_flags; }
...
/** Returns a Flags object that consists of the bitwise-or of \a lhs and \a
    rhs. */
template <class FlagType> Flags<FlagType> operator|( Flags<FlagType> lhs, Flags<FlagType> rhs )
{
...
/** Returns a Flags object that consists of the bitwise-or of \a lhs and \a
    rhs. */
template <class FlagType> Flags<FlagType> operator|( Flags<FlagType> lhs, FlagType rhs )
{
...
/** Returns a Flags object that consists of the bitwise-or of \a lhs and \a
    rhs. */
template <class FlagType> Flags<FlagType> operator|( FlagType lhs, Flags<FlagType> rhs )
{
...
typename std::enable_if<is_flag_type<FlagType>::value, Flags<FlagType>>::type operator|( FlagType lhs, FlagType rhs )
{
...
template <class FlagType> Flags<FlagType> operator~( Flags<FlagType> flags )
{
...
template <class FlagType>
typename std::enable_if<is_flag_type<FlagType>::value, Flags<FlagType>>::type operator~( FlagType flag )
{
