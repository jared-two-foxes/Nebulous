...
public:
  ...
  */
  template <typename T> static inline bool IsPO2( T n ) { return ( n & ( n - 1 ) ) == 0; }

  /** Returns the number of bits a pattern must be shifted right by to
      remove right-hand zeros.
  */
  template <typename T> static inline unsigned int GetBitShift( T mask )
  {
    if ( mask == 0 )
    {
      return 0;
  ...
  */
  ...
  static inline unsigned int IntRead( const void* src, int n )
  {
    ...
    case 3:
#if NE_ENDIAN == NE_ENDIAN_BIG
      return ( (uint32)( (uint8*)src )[0] << 16 ) | ( (uint32)( (uint8*)src )[1] << 8 ) |
             ( (uint32)( (uint8*)src )[2] );
#else
      return ( (uint32)( (uint8*)src )[0] ) | ( (uint32)( (uint8*)src )[1] << 8 ) |
