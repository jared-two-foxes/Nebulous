...
/**@brief The Quaternion implements quaternion to perform linear algebra rotations in combination with Matrix3, Vector4
 * and Transform.
 */
class Quaternion
{
public:
  ...
  /**@brief Return the quaternion which is the result of Spherical Linear Interpolation between this and the other
   * quaternion
...
template <> inline Assert& Assert::Variable( const char* const name, const Quaternion& value )
{
  printf( "  o Variable %s = <%f,%f,%f,%f> (quaternion)\n", name, value.m_vec.x, value.m_vec.y, value.m_vec.w,
          value.m_vec.z );
  return *this;
}
...
inline Quaternion::Quaternion( const Quaternion& rhs ) { m_vec = rhs.m_vec; }
...
inline Quaternion::Quaternion( const Vector4& _axis, const Real& _angle ) { setRotation( _axis, _angle ); }

inline Quaternion::Quaternion( const Real& yaw, const Real& pitch, const Real& roll ) { setEuler( yaw, pitch, roll ); }

inline void Quaternion::setValue( Real _x, Real _y, Real _z, Real _w ) { m_vec.set( _x, _y, _z, _w ); }
...
inline void Quaternion::setRotation( const Vector4& axis, const Real& _angle )
{
  ...
  setValue( axis.x * s, axis.y * s, axis.z * s, cos( _angle * Real( 0.5 ) ) );
}
...
inline Quaternion& Quaternion::operator*=( const Quaternion& q )
{
  setValue( m_vec.w * q.m_vec.x + m_vec.x * q.m_vec.w + m_vec.y * q.m_vec.z - m_vec.z * q.m_vec.y,
...
{
  return m_vec.x * q.m_vec.x + m_vec.y * q.m_vec.y + m_vec.z * q.m_vec.z + m_vec.w * q.m_vec.w;
}

inline Real Quaternion::length2() const { return dot( *this ); }

inline Real Quaternion::length() const { return sqrt( length2() ); }

inline Quaternion& Quaternion::normalize() { return *this /= length(); }
...
inline Quaternion Quaternion::inverse() const { return Quaternion( -m_vec.x, -m_vec.y, -m_vec.z, m_vec.w ); }

inline Quaternion Quaternion::operator+( const Quaternion& q2 ) const
{
  const Quaternion& q1 = *this;
  return Quaternion( q1.m_vec.x + q2.m_vec.x, q1.m_vec.y + q2.m_vec.y, q1.m_vec.z + q2.m_vec.z,
                     q1.m_vec.w + q2.m_vec.w );
}
...
inline Quaternion Quaternion::slerp( const Quaternion& q, const Real& t ) const
{
  ...
  if ( fabs( product ) != Real( 1 ) )
  {
    ...
    return Quaternion( ( m_vec.x * s0 + q.m_vec.x * s1 ) * d, ( m_vec.y * s0 + q.m_vec.y * s1 ) * d,
                       ( m_vec.z * s0 + q.m_vec.z * s1 ) * d, ( m_vec.z * s0 + q.m_vec.w * s1 ) * d );
  }
  else
  {
    return *this;
