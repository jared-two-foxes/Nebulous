...
#ifndef NEBULAE_COMMON_MATH_TRANSFORM_H_
#define NEBULAE_COMMON_MATH_TRANSFORM_H_
...
public:
  ...
  explicit Transform( const Quaternion& q, const Vector4& c ) : m_basis( q ), m_origin( c ) {}

  /**@brief Constructor from btMatrix3x3
   * @param b Rotation from Matrix
   * @param c Translation from Vector*/
  explicit Transform( const Matrix3& b, const Vector4& c ) : m_basis( b ), m_origin( c ) {}

  /**@brief Copy constructor */
  Transform( const Transform& other ) : m_basis( other.m_basis ), m_origin( other.m_origin ) {}
  ...
  {
    return Vector4( m_basis[0].dot( x ) + m_origin.x, m_basis[1].dot( x ) + m_origin.y,
                    m_basis[2].dot( x ) + m_origin.z );
  }

  /**@brief Return the transform of the vector */
  Vector4 operator*( const Vector4& x ) const { return ( *this )( x ); }
  ...
  void setOrigin( const Vector4& origin ) { m_origin = origin; }

  Vector4 invXform( const Vector4& inVec ) const;


  /**@brief Set the rotational element by btMatrix3x3 */
  void setBasis( const Matrix3& basis ) { m_basis = basis; }

  /**@brief Set the rotational element by btQuaternion */
  void setRotation( const Quaternion& q ) { m_basis.setRotation( q ); }
...
inline Transform Transform::inverseTimes( const Transform& t ) const
{
  ...
  Matrix3 m = m_basis;
  m.transpose();
  return Transform( m * t.m_basis, v * m_basis );
}
...
#endif // NEBULAE_COMMON_MATH_TRANSFORM_H_
