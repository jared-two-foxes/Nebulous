...
template <> inline Assert& Assert::Variable( const char* const name, const Vector4& value )
{
...
inline Vector4::Vector4( Real _x, Real _y, Real _z, Real _w ) : x( _x ), y( _y ), z( _z ), w( _w ) {}
...
inline Real Vector4::operator[]( int index ) const { return ( &x )[index]; }

inline Real& Vector4::operator[]( int index ) { return ( &x )[index]; }

inline Real* Vector4::ptr() { return &x; }

inline const Real* Vector4::ptr() const { return &x; }
...
inline Real Vector4::length() const { return sqrtf( x * x + y * y + z * z + w * w ); }

inline Real Vector4::distance( const Vector4& vec ) const
{
...
inline Real Vector4::dot( const Vector4& rhs ) const { return ( x * rhs.x + y * rhs.y + z * rhs.z + w * rhs.w ); }
