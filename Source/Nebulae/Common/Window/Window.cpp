...
void Window::MoveAndResize( int x, int y, int w, int h )
{
  m_position.x = x;
  m_position.y = y;
  m_size.x = w;
...
void Window::SetCaption( const wchar_t* szCaption ) { m_caption = szCaption; }

bool Window::IsPointInWindow( const Point& point ) const
{
  Point pos( point );
  pos -= m_position;
...
void Window::AddInputListener( InputListener* pInputListener ) { m_listeners.push_back( pInputListener ); }

void Window::RemoveInputListener( InputListener* pInputListener ) { m_listeners.remove( pInputListener ); }
