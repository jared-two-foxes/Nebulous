...
uint16 WindowsKeyFromVirtualKey( KeyCode keyCode )
{
  uint16 retval = 0;
  ...
  case VKC_TAB:
    retval = VK_TAB;
    break;
  case VKC_LEFTBRACKET:
    retval = VK_OEM_4;
    break;
  case VKC_RIGHTBRACKET:
    retval = VK_OEM_6;
    break;
  case VKC_RETURN:
    retval = VK_RETURN;
  ...
  case VKC_COMMA:
    ...
    break;
  case VKC_PERIOD:
    retval = VK_OEM_PERIOD;
    break;
  case VKC_RSHIFT:
    retval = VK_RSHIFT;
    break;
  case VKC_KP_MULTIPLY:
    retval = VK_MULTIPLY;
    break;
  case VKC_LALT:
    retval = VK_LMENU;
...
void SetKeyState( uint8* keyboardState, KeyCode virtualKey, bool down, bool toggled )
{
  if ( toggled )
  {
...
bool Keyboard::IsKeyUp( KeyCode virtualKey ) const
{
  return !std::bitset<sizeof( uint8 ) * CHAR_BIT>( m_keyboardState[virtualKey] ).test( 7 );
}

bool Keyboard::IsToggled( KeyCode virtualKey ) const
{
  return std::bitset<sizeof( uint8 ) * CHAR_BIT>( m_keyboardState[virtualKey] ).test( 0 );
}

void Keyboard::SetKeyState( KeyCode virtualKey, bool down )
{
...
void Keyboard::GetAllStates( uint8* state ) { memcpy( state, m_keyboardState, 256 ); }
...
int32 Keyboard::ToUnicode( KeyCode keycode, uint32* unicodeChar )
{
  BYTE kb[256];
