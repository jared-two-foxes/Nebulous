...
template <typename T> struct json_caster
{
  static T to_value( const Json::Value& value, const T& valueOnError )
  {
    return T::no_valid_json_cast_provided_for_type; // The general implementation is deliberately left blank; you cannot
                                                    // cast this type.
  }
};

template <> struct json_caster<bool>
{
...
template <> struct json_caster<int>
{
...
template <> struct json_caster<uint32>
{
  static uint32 to_value( const Json::Value& value, uint32 valueOnError )
  {
    NE_ASSERT( value.isUInt() || value.isInt(), "Bad json_cast; expected unsigned integer, got '%s'",
...
template <> struct json_caster<Real>
{
  static float to_value( const Json::Value& value, Real valueOnError )
  {
    ...
    NE_ASSERT( value.asDouble() <= std::numeric_limits<float>::max(),
               "Bad json_cast; real value is out of range for a float, value may not be larger than '%f'",
...
template <> struct json_caster<const char*>
{
...
template <> struct json_caster<std::string>
{
...
template <> struct json_caster<Vector2>
{
...
template <> struct json_caster<Vector4>
{
  static Vector4 to_value( const Json::Value& value, const Vector4& valueOnError )
  {
    NE_ASSERT( value.isArray(), "Bad json_cast; expected array, found '%s'", value.toStyledString().c_str() );
    NE_ASSERT( value.size() <= 4, "Invalid vector array in json data. We do not support %d-dimensional vectors",
               value.size() );
...
template <> struct json_caster<Quaternion>
{
...
template <typename T> struct json_caster<std::vector<T>>
{
...
template <typename T> T Nebulae::json_cast( const Json::Value& value )
{
  return Nebulae::json_caster<T>::to_value( value, T() );
}


template <typename T> T Nebulae::json_cast_with_default( const Json::Value& value, const T& defaultValue )
{
  if ( value.isNull() )
  {
    return defaultValue;
