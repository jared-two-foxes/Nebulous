...
size_t ZipFile::Write( const void* buffer, size_t length )
{
  // Tots not supported right now!
  return 0;
}

void ZipFile::Seek( size_t position ) { m_pos = std::min<std::size_t>( position, m_buffer.size() ); }

void ZipFile::SeekToEnd() { m_pos = m_buffer.size(); }

void ZipFile::Skip( size_t bytes ) { m_pos = std::min<std::size_t>( m_pos + bytes, m_buffer.size() ); }

size_t ZipFile::Tell() const { return m_pos; }
...
//   uLong           ratio     = 0;
//   uint32          total_out = 0;
//   int32           err       = unzGetCurrentFileInfo64(uf, &file_info, filename_inzip, sizeof(filename_inzip), NULL,
//   0, NULL, 0 ); NE_ASSERT( err == UNZ_OK, "Error %d with zipfile in unzGetCurrentFileInfo\n", err); if( err != UNZ_OK
//   )
//   {
//     return err;
//   }
