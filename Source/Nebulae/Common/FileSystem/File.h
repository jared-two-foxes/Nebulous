...
class File
{
  public:
    ...
    virtual size_t Read([[maybe_unused]] void* buffer, [[maybe_unused]] size_t size) { return static_cast<size_t>(-1); }
   
    /// Synchronously writes from a buffer into the file. Returns the number of bytes written, or 0 if the operation failed.
    virtual size_t Write([[maybe_unused]] const void* buffer, [[maybe_unused]] size_t size) { return static_cast<size_t>(-1); }
