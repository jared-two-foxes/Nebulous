...
GLES2HardwareBufferImpl::GLES2HardwareBufferImpl( const Flags<HardwareBufferUsage>& usage, std::size_t sizeInBytes,
...
void GLES2HardwareBufferImpl::Bind()
{
...
bool GLES2HardwareBufferImpl::Load()
{
...
void GLES2HardwareBufferImpl::WriteData( std::size_t offset, std::size_t length, const void* pSource,
                                         bool discardWholeBuffer )
{
  ...
  glBufferData( m_iOpenGlBindFlags, (GLsizeiptr)length, pBuffer,
                m_iOpenGlUsage ); //< this could be using glBufferSubData for existing buffers?
  CheckForGLESError();
}

void* GLES2HardwareBufferImpl::Lock( std::size_t offset, size_t length, LockOptions options )
{
  void* retptr = nullptr;
...
void GLES2HardwareBufferImpl::Unlock()
{
  // Bind();
...
GLenum GLES2HardwareBufferImpl::GetGLUsage( const Flags<HardwareBufferUsage>& usage )
{
