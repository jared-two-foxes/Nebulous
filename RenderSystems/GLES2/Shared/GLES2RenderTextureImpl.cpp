...
GLES2RenderTextureImpl::GLES2RenderTextureImpl( int32 width, int32 height )
  : RenderTextureImpl( width, height ), m_framebuffer( 0 ), m_colourBuffer( nullptr ), m_depthBuffer( nullptr )
{
}
...
void GLES2RenderTextureImpl::BindRenderTexture() { glBindFramebuffer( GL_FRAMEBUFFER, m_framebuffer ); }

bool GLES2RenderTextureImpl::BindColourBuffer( int32 index, int32 width, int32 height ) { return false; }

bool GLES2RenderTextureImpl::BindColourTexture( int32 index, Texture* texture )
{
  ...
  if ( texture->GetTextureType() == TextureType::TEX_TYPE_CUBE_MAP )
  {
    ...
    // pRenderTexture->face, pRenderTexture->color->handle, 0);
  }
  else
  {
...
bool GLES2RenderTextureImpl::BindDepthBuffer( int32 index, int32 width, int32 height )
{
...
bool GLES2RenderTextureImpl::BindDepthTexture( Texture* texture )
{
  NE_ASSERT( m_depthBuffer == nullptr, "Attempting to set a texture to the depth slot which is already occupied" );
  NE_ASSERT( texture->GetTextureType() == TextureType::TEX_TYPE_2D,
             "Attempting to set a texture to the depth slot which is the incorrect format" );
...
bool GLES2RenderTextureImpl::Load()
{
  glGenFramebuffers( 1, &m_framebuffer );
  return false;
}

bool GLES2RenderTextureImpl::Unload()
{
