...
GLES2TextureImpl::GLES2TextureImpl( const std::string& strFileName ) : TextureImpl( strFileName ), m_iHandle( 0 ) {}


GLES2TextureImpl::~GLES2TextureImpl() {}


bool GLES2TextureImpl::Load( const Image& image )
{
  ...
  // gles support only a very small subset of image data types.  Particularly it will not support anything thats not
  // GL_UNSIGNED_BYTE so convert the image.
  ...
  glTexImage2D( GL_TEXTURE_2D, 0,
                srcFormat, // glformat,
                (GLsizei)m_width, (GLsizei)m_height, 0, srcFormat, srcDataType, srcData );
...
bool GLES2TextureImpl::LoadFromMemory( unsigned char* szBuffer, uint8 srcDataType, uint8 channels, int iWidth,
                                       int iHeight )
{
  m_width = iWidth;
  m_height = iHeight;
