...
const std::string& RenderSystem_GLES2_Win32::GetName() const { return m_name; }

bool RenderSystem_GLES2_Win32::Initiate()
{
  ...
  EGLint attributes[] = { EGL_RENDERABLE_TYPE,
                          EGL_OPENGL_ES2_BIT /*| EGL_OPENGL_ES3_BIT_KHR*/, // Request OpenGL ES 2.0 configs
                          // EGL_SURFACE_TYPE, EGL_WINDOW_BIT,
                          EGL_DEPTH_SIZE, 16, EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_NONE };
  EGLConfig configs[1];
  EGLint num_configs;
...
void RenderSystem_GLES2_Win32::Clear() { glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT ); }

void RenderSystem_GLES2_Win32::SetClearColour( float r, float g, float b, float a ) { glClearColor( r, g, b, a ); }

void RenderSystem_GLES2_Win32::SwapBuffers() { eglSwapBuffers( m_display, m_surface ); }

void RenderSystem_GLES2_Win32::SetBlendingState( bool enable )
{
  if ( enable )
  {
    glEnable( GL_BLEND );
...
void RenderSystem_GLES2_Win32::SetDepthTest( bool enable )
{
  if ( enable )
  {
    glEnable( GL_DEPTH_TEST );
...
void RenderSystem_GLES2_Win32::BeginScissorClipping( Point ul, Point lr )
{
  ...
  glScissor( Value( ul.x ), Value( m_window->GetHeight() - lr.y ), Value( lr.x - ul.x ), Value( lr.y - ul.y ) );

  m_scissor_clipping_rects.push_back( Rect( ul, lr ) );
}

void RenderSystem_GLES2_Win32::EndScissorClipping()
{
  ...
  {
    const Rect& r = m_scissor_clipping_rects.back();
    glScissor( Value( r.GetLeft() ), Value( m_window->GetHeight() - r.GetBottom() ), Value( r.GetWidth() ),
               Value( r.GetHeight() ) );
  }
}

void RenderSystem_GLES2_Win32::BeginStencilClipping( Point inner_ul, Point inner_lr, Point outer_ul, Point outer_lr )
{
  //  if (!g_stencil_bit) {
...
void RenderSystem_GLES2_Win32::EndStencilClipping()
{
...
HardwareBufferImpl* RenderSystem_GLES2_Win32::CreateBufferImpl( const Flags<HardwareBufferUsage>& usage,
                                                                std::size_t sizeInBytes,
                                                                HardwareBufferBinding bindFlags, void* sysMem )
{
  return new GLES2HardwareBufferImpl( usage, sizeInBytes, bindFlags, sysMem );
}

HardwareShaderImpl* RenderSystem_GLES2_Win32::CreateShaderImpl( const std::string& strFileName,
                                                                HardwareShaderType eType )
{
  return new GLES2HardwareShaderImpl( strFileName, eType );
}

InputLayoutImpl* RenderSystem_GLES2_Win32::CreateInputLayoutImpl( const VertexDeceleration* pVertexDecl,
                                                                  const HardwareShader* pVertexShader )
{
  return new GLES2InputLayoutImpl( pVertexDecl, pVertexShader );
}

RenderTextureImpl* RenderSystem_GLES2_Win32::CreateRenderTextureImpl( int32 width, int height )
{
  return new GLES2RenderTextureImpl( width, height );
}

Sampler::Impl* RenderSystem_GLES2_Win32::CreateSamplerImpl() { return new GLES2SamplerImpl(); }

TextureImpl* RenderSystem_GLES2_Win32::CreateTextureImpl( const std::string& strName )
{
  return new GLES2TextureImpl( strName );
}

void RenderSystem_GLES2_Win32::SetVertexBuffers( int iSlot, HardwareBuffer* pBufferImpl, size_t iStride,
                                                 size_t iOffset )
{
...
void RenderSystem_GLES2_Win32::SetInputLayout( InputLayout* pInputLayout )
{
...
void RenderSystem_GLES2_Win32::SetIndexBuffer( HardwareBuffer* pBufferImpl, size_t iOffset )
{
  // @note [jared.watt] Should check that this is actually a vertex buffer.
  static_cast<GLES2HardwareBufferImpl*>( pBufferImpl->GetImpl() )->Bind();
}

void RenderSystem_GLES2_Win32::SetRenderTexture( RenderTexture* renderTexture )
{
...
void RenderSystem_GLES2_Win32::SetShaders( HardwareShader* vertexShader, HardwareShader* fragmentShader )
{
  NE_ASSERT( vertexShader->GetImpl()->GetType() == VERTEX_SHADER,
             "Shader passed in the vertexShader slot is not a Vertex Shader" );
  NE_ASSERT( fragmentShader->GetImpl()->GetType() == PIXEL_SHADER,
             "Shader passed in the fragmentShader slot is not a Fragement Shader" );
...
void RenderSystem_GLES2_Win32::SetOperationType( OperationType eType )
{
  switch ( eType )
  {
  case OT_POINTS:
...
void RenderSystem_GLES2_Win32::Draw( std::size_t iVertexCount, std::size_t iStartVertexLocation )
{
  glDrawArrays( m_OperationMode, (int)iStartVertexLocation, (int)iVertexCount );
  CheckForGLESError();
}

void RenderSystem_GLES2_Win32::DrawIndexed( std::size_t iIndexCount, std::size_t iStartIndexLocation,
...
void RenderSystem_GLES2_Win32::SetBufferBinding( uint32 iTarget, uint32 iIndex, HardwareBuffer* pImpl )
{
  NE_ASSERT( false, "Not yet implemented!" );
}

UniformDefinition RenderSystem_GLES2_Win32::GetUniformByName( const char* name ) const
{
...
void RenderSystem_GLES2_Win32::SetSamplerBinding( uint32 iTarget, uint32 iIndex, Sampler* sampler ) {}

void RenderSystem_GLES2_Win32::SetTextureBinding( uint32 iTarget, uint32 iIndex, Texture* texture ) {}

void RenderSystem_GLES2_Win32::SetUniformBinding( UniformDefinition& definition, void* value )
{
