...
bool InputLayoutImpl_OGL::Load() { return true; }
...
void InputLayoutImpl_OGL::Bind( ProgramObject* programObject )
{
  ...
  for ( int i = 0, n = m_vertexDecl->GetElementCount(); i < n; ++i )
  {
    ...
    if ( pElement->Semantic == VES_POSITION )
    {
      ...
      {
        glVertexAttribPointer( iPositionLocation, iSize, iFormat, GL_FALSE, (GLsizei)iStride,
                               BUFFER_OFFSET( pElement->AlignedByteOffset ) );
    ...
    else if ( pElement->Semantic == VES_NORMAL )
    {
      ...
      {
        glVertexAttribPointer( iNormalLocation, iSize, iFormat, GL_FALSE, (GLsizei)iStride,
                               BUFFER_OFFSET( pElement->AlignedByteOffset ) );
    ...
    else if ( pElement->Semantic == VES_TEXTURE_COORDINATES )
    {
      ...
      {
        glVertexAttribPointer( iTexCoordLocation, iSize, iFormat, GL_FALSE, (GLsizei)iStride,
                               BUFFER_OFFSET( pElement->AlignedByteOffset ) );
    ...
    else if ( pElement->Semantic == VES_DIFFUSE )
    {
      ...
      {
        glVertexAttribPointer( iDiffuseLocation, iSize, iFormat, GL_FALSE, (GLsizei)iStride,
                               BUFFER_OFFSET( pElement->AlignedByteOffset ) );
