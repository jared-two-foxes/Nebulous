...
UniformType ConvertGLUnformTypeToNebulaeType( GLenum type )
{
  switch ( type )
  {
  case GL_FLOAT:
...
ProgramObject::ProgramObject( HardwareShader* vertexShader, HardwareShader* fragmentShader )
  : m_handle( 0 ), m_vertexShader( vertexShader ), m_fragmentShader( fragmentShader )
{
}
...
GLuint ProgramObject::GetHandle() const { return m_handle; }


HardwareShader* ProgramObject::GetVertexShader() const { return m_vertexShader; }
...
bool ProgramObject::Load()
{
  ...
  // Since OpenGL isnt guarenteed to finalise the generation of the shader program until its 'bound' we will fake bind
  // it now.
  ...
  {
    GLenum type = GL_ZERO;
    GLint nameLen = -1, num = -1;
    char name[100];

    glGetActiveUniform( m_handle, GLuint( i ), sizeof( name ) - 1, &nameLen, &num, &type, name );
