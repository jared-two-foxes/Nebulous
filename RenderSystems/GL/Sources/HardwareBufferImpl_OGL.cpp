...
HardwareBufferImpl_OGL::HardwareBufferImpl_OGL( const Flags<HardwareBufferUsage>& usage, std::size_t sizeInBytes,
                                                HardwareBufferBinding bindFlags, void* sysMem )
  : HardwareBufferImpl( usage, sizeInBytes, bindFlags, sysMem ), m_iHandle( 0 ), m_iOpenGlBindFlags( 0 )
{
}
...
void HardwareBufferImpl_OGL::Bind()
{
...
bool HardwareBufferImpl_OGL::Load()
{
...
void HardwareBufferImpl_OGL::WriteData( std::size_t offset, std::size_t length, const void* pSource,
                                        bool discardWholeBuffer )
{
  ...
  glBufferData( m_iOpenGlBindFlags, length, pBuffer,
                m_iOpenGlUsage ); //< this could be using glBufferSubData for existing buffers?
  CheckForGLError();
}


void* HardwareBufferImpl_OGL::Lock( std::size_t offset, size_t length, LockOptions options )
{
  void* retptr = nullptr;
...
void HardwareBufferImpl_OGL::Unlock()
{
  Bind();
