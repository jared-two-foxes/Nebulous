...
TextureImpl_OGL::TextureImpl_OGL( const std::string& strFileName ) : TextureImpl( strFileName ), m_iHandle( 0 ) {}


TextureImpl_OGL::~TextureImpl_OGL() {}


bool TextureImpl_OGL::Load( const Image& image )
{
  ...
  glTexImage2D( GL_TEXTURE_2D, 0, glformat, (GLsizei)m_width, (GLsizei)m_height, 0, srcFormat, srcDataType,
                image.getData() );
  CheckForGLError();
...
bool TextureImpl_OGL::LoadFromMemory( unsigned char* pImageData, uint8 srcDataType, uint8 channels, int iWidth,
                                      int iHeight )
///
/// @remarks
///   We Are Using GL_LUMINANCE_ALPHA To Indicate that We Are Using 2 Channel Data. Used when loading in font glyph
///   data.
///
{
  ...
  {
  case 1:
    srcStructure = GL_LUMINANCE;
    break;
  case 2:
    srcStructure = GL_LUMINANCE_ALPHA;
    break;
  case 4:
    srcStructure = GL_BGRA;
    break;
