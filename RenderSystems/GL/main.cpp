...
NE_PLUGIN_EXPORT Nebulae::RenderSystem* CreateRenderer( std::shared_ptr<Nebulae::FileSystem> fileSystem,
                                                        std::shared_ptr<Nebulae::Window> window )
{
  return new Nebulae::RenderSystem_OGL( fileSystem, window );
}

NE_PLUGIN_EXPORT void DestroyRenderer( Nebulae::RenderSystem* renderer )
{
  delete renderer;
  renderer = nullptr;
}
