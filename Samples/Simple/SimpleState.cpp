...
SimpleState::SimpleState( const std::string& name ) : State( name ), m_pCamera( NULL ) {}

SimpleState::~SimpleState() {}

void SimpleState::Enter( Nebulae::StateStack* caller )
{
  ...
  static const Real fVertices[] = { -50.0f, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f,
                                    50.0f,  0.0f, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f };

  // Attempt to create a Buffer of video memory
  HardwareBuffer* pBuffer = m_pRenderSystem->CreateBuffer( "DebugBuffer", HBU_STATIC, 14 * sizeof( Real ),
                                                           HardwareBufferBinding::HBB_VERTEX, (void*)&fVertices[0] );

  // Attempt to create a vertex shader.
  HardwareShader* pVertexShader = m_pRenderSystem->CreateShader( "debug_vs.glsl", VERTEX_SHADER );

  // Attempt to create a fragment shader.
  HardwareShader* pPixelShader = m_pRenderSystem->CreateShader( "debug_fs.glsl", PIXEL_SHADER );
  ...
  InputLayout* pInputLayout = m_pRenderSystem->CreateInputLayout( "DebugLayout", pVertexDecl, pVertexShader );
}

void SimpleState::Exit( Nebulae::StateStack* caller ) { (void)caller; }

void SimpleState::Update( float fDeltaTimeStep, Nebulae::StateStack* pCaller )
{
  (void)fDeltaTimeStep;
  (void)pCaller;
}

void SimpleState::Render() const
{
