...
void PlayButtonPressed( void* pOwner ) { printf( "%s", "The play button was pressed!\n" ); }

void HighScoreButtonPressed( void* pOwner ) { printf( "%s", "The high score button was pressed!\n" ); }

void OptionsButtonPressed( void* pOwner ) { printf( "%s", "The options button was pressed!\n" ); }

void AboutButtonPressed( void* pOwner ) { printf( "%s", "The about button was pressed!\n" ); }


GuiSampleState::GuiSampleState( const std::string& name )
  : State( name ), m_pCamera( NULL ), m_pGuiManager( NULL ), m_inputListener( NULL ), m_fontMgr( NULL )
{
}

GuiSampleState::~GuiSampleState() {}

void GuiSampleState::SetupInternalSystems( StateStack* caller )
{
...
void GuiSampleState::SetupGuiComponents( StateStack* caller )
{
  ...
  TextControl* pLabel1 =
    factory->CreateTextControl( 0, 600, 200, 100, "The quick brown fox jumps over the lazy dog!", arialPtr, CLR_YELLOW,
                                FORMAT_TOP | FORMAT_LEFT | FORMAT_WORDBREAK );
  TextControl* pLabel2 =
    factory->CreateTextControl( 440, 600, 200, 100, "The quick brown fox jumps over the lazy dog!", groboldPtr,
                                CLR_BLUE, FORMAT_TOP | FORMAT_RIGHT | FORMAT_WORDBREAK );
  TextControl* pLabel3 =
    factory->CreateTextControl( 220, 700, 200, 100, "The quick brown fox jumps over the lazy dog!", celestiaPtr,
                                CLR_WHITE, FORMAT_BOTTOM | FORMAT_CENTER | FORMAT_WORDBREAK );
...
void GuiSampleState::Enter( StateStack* caller )
{
  SetupInternalSystems( caller );
  SetupGuiComponents( caller );
}

void GuiSampleState::Exit( StateStack* caller )
{
  if ( m_pGuiManager )
  {
...
void GuiSampleState::Update( float fDeltaTimeStep, StateStack* pCaller ) {}

void GuiSampleState::Render() const
{
  if ( m_renderer != NULL )
  {
