...
public:
  ...
  template <class T> T* GetComponent( int32 identifier ) const;

  template <class T> int32 GetComponents( std::vector<int32>& identifiers, std::vector<T*>* components ) const;

  template <class T> int32 GetNextAvailableIndex() const;

  template <class T> bool ReleaseComponent( int32 identifier );

private:
  ComponentPool* GetComponentPool( ComponentType type ) const;
};

template <class T> T* ComponentCache::GetComponent( int32 identifiers ) const
{
...
template <class T>
int32 ComponentCache::GetComponents( std::vector<int32>& identifiers, std::vector<T*>* components ) const
{
  int32 count = 0;
  ComponentPool* pool = GetComponentPool( T::GetType() );
...
template <class T> int32 ComponentCache::GetNextAvailableIndex() const
{
  ComponentPool* pool = GetComponentPool( T::GetType() );
  if ( pool != NULL )
  {
