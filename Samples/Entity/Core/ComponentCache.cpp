...
template <class T> class ComponentPoolImpl : public ComponentPool
{
private:
...
public:
  ...
  const int32 GetNextAvailableIndex() const { return m_unusedIndices.empty() ? -1 : m_unusedIndices.back(); }
};
...
void ComponentCache::Init( int count )
///
...
void ComponentCache::AddPoolForComponentType( ComponentType type )
{
  (void)type;
  // ComponentPool* pool = NULL; //new ComponentPoolImpl<type>();
}

ComponentPool* ComponentCache::GetComponentPool( ComponentType type ) const
{
