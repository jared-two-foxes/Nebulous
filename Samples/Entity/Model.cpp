...
Model::Model( std::shared_ptr<TemplateCache>& templateCache ) : m_templates( templateCache ), m_entities() {}
...
ComponentCache& Model::GetComponentCache()
///
...
const std::vector<Entity*>& Model::GetEntities() const
///
...
std::shared_ptr<TemplateCache> Model::GetTemplates()
///
...
Entity* Model::CreateEntity( const char* templateName, const char* uniqueName )
///
...
Entity* Model::FindEntityByName( const char* uniqueName ) const
///
...
void Model::DestroyEntity( Entity* entity )
///
...
void Model::LoadSceneFile( File& stream )
  ...
  if ( success )
  {
    ...
    for ( uint32 i = 0, n = entities.size(); i < n; ++i )
    {
      ...
      if( motionComponent != NULL ) {
        ...
        motionComponent->maximumVelocity = json_cast_with_default<Real>(    motionData["maximumVelocity"],
      maximumVelocity ); motionComponent->acceleration    = json_cast_with_default<Vector4>( motionData["acceleration"],
      Vector4(0,0,0,0) );
      }

      std::vector<PlacementComponent*> placementComponents = m_components.GetComponents<PlacementComponent>( identifier
      ); if( !placementComponents.empty() ) { Json::Value& placementData = componentData["placement"];

        placementComponents[0]->position = json_cast_with_default<Vector4>(    placementData["position"],
      Vector4(0,0,0,0) ); placementComponents[0]->rotation = json_cast_with_default<Quaternion>(
      placementData["rotation"], Quaternion(0,0,0,1) );
      }

      std::vector<PlacementConstraintComponent*> placementConstraintComponents =
      m_components.GetComponents<PlacementConstraintComponent>( identifier ); if( !placementConstraintComponents.empty()
      ) { Json::Value& placementConstraintData = componentData["placementConstraint"];

        placementConstraintComponents[0]->leftBound   = json_cast_with_default<Real>(
      placementConstraintData["leftBound"],   std::numeric_limits<Real>::lowest() );
        placementConstraintComponents[0]->rightBound  = json_cast_with_default<Real>(
      placementConstraintData["rightBound"],  std::numeric_limits<Real>::max() );
        placementConstraintComponents[0]->topBound    = json_cast_with_default<Real>(
      placementConstraintData["topBound"],    std::numeric_limits<Real>::max() );
        placementConstraintComponents[0]->bottomBound = json_cast_with_default<Real>(
      placementConstraintData["bottomBound"], std::numeric_limits<Real>::lowest() );
        placementConstraintComponents[0]->backBound   = json_cast_with_default<Real>(
      placementConstraintData["backBound"],   std::numeric_limits<Real>::lowest() );
        placementConstraintComponents[0]->frontBound  = json_cast_with_default<Real>(
      placementConstraintData["frontBound"],  std::numeric_limits<Real>::max() );
      }

      std::vector<CollisionComponent*> collisionComponents = m_components.GetComponents<CollisionComponent>( identifier
      ); if( !collisionComponents.empty() ) { Json::Value& collisionData = componentData["collision"];
...
void Model::ApplyTemplate( Entity& entity, EntityTemplate& entityTemplate )
///
