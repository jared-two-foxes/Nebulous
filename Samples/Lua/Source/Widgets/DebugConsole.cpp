...
const char* lua_function_myprint = "function myprint(...) \n"
                                   "  local str \n"
                                   "  str = '' \n"
                                   " \n"
...
const std::vector<std::string>& DebugConsole::Lines() const { return m_lines; }
...
void DebugConsole::AddLine( const std::string& line ) { m_lines.push_back( rtrim( line ) ); }

void DebugConsole::PushToGui()
{
  ...
  std::for_each( m_lines.end() - rows_to_show, m_lines.end(),
                 [&]( std::string& str )
                 {
                   textToPush += str;
                   textToPush += "\n";
...
void DebugConsole::KeyPressed( KeyCode keyCode, Flags<ModKey> modKeys )
{
...
void DebugConsole::KeyReleased( KeyCode keyCode, Flags<ModKey> modKeys ) {}

int DebugConsole::InsertOutputFromLua( lua_State* L )
{
...
int DebugConsole::ClearConsoleFromLua( lua_State* L )
{
  // The interpreter is not accessable at this point so duplicating code from "LuaInterpreter::GetUserData" is
  // unfortunately unavoidable.
  lua_pushstring( L, "console" );
...
void DebugConsole::AddToHistory( const std::string& currentLine )
{
