...
void ExampleApplication::Init()
{
  ...
  m_batcher->Init();
  m_widgetRenderer =
    std::make_shared<Nebulae::WidgetRenderer>( GetPlatform()->GetFileSystem(), m_batcher.get(), m_pRenderSystem );
#else
  m_widgetRenderer =
    std::make_shared<Nebulae::WidgetRenderer>( GetPlatform()->GetFileSystem(), nullptr, m_pRenderSystem );
#endif
  ...
  TextControl* consoleTextElement = m_guiController->GetWidgetFactory()->CreateTextControl(
    0, 0, 800, 200, "Initial Line of text!", font, CLR_WHITE, FORMAT_TOP | FORMAT_LEFT );
  m_debugConsole =
    std::make_shared<Nebulae::DebugConsole>( m_scriptInterpreter, consoleTextElement, GetPlatform()->GetKeyboard() );
...
void ExampleApplication::ProcessSimulation( uint64 elapsed )
{
#ifdef NEBULAE_AUDIO
...
void ExampleApplication::ProcessFrame( uint64 elapsed )
{
