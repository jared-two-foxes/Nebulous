...
ExampleScreen::ExampleScreen( const std::string& name, RenderSystemPtr renderSystem )
  : State( name ), m_renderSystem( renderSystem ), m_rotation( 0.0f )
{
}
...
void ExampleScreen::Enter( StateStack* /*caller*/ )
{
  //
  // Setup camera.
  //
  ...
  if ( NULL == buffer )
  {
    ...
    buffer = m_renderSystem->CreateBuffer( "CubeBuffer", HBU_STATIC, sizeof( gCubeVertexData_v2 ),
...
void ExampleScreen::Exit( StateStack* /*caller*/ ) {}


void ExampleScreen::Update( float fDeltaTimeStep, StateStack* /*pCaller*/ )
{
...
void ExampleScreen::Render() const
{
